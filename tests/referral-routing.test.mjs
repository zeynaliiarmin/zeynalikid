...
globalThis.fetch=async()=>new Response(JSON.stringify({settings:{consultants,courseTabs:tabs}}),{status:200,headers:{'Content-Type':'application/json'}});
...
const invalidResponse=await invoke('mohz1');if(invalidResponse.statusCode!==404||!invalidResponse.body.includes('class="number"')||!invalidResponse.body.includes('دسترسی سریع'))throw new Error('Handler rich 404 failed.');
globalThis.fetch=originalFetch;

const handlerSource=await readFile(new URL('../api/referral/[code].js',import.meta.url),'utf8');
if(!handlerSource.includes('class=\"number\"')||!handlerSource.includes('دسترسی سریع')||!handlerSource.includes('parseServerReferral'))throw new Error('Server 404/referral handler lost the rich 404 contract.');
