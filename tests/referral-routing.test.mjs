...
const invalidResponse=await invoke('mohz1');if(invalidResponse.statusCode!==404||!invalidResponse.body.includes('class="nf-art"')||!invalidResponse.body.includes('دسترسی سریع')||!invalidResponse.body.includes('height:100dvh'))throw new Error('Handler fixed-viewport 404 failed.');
...
if(!static404.includes('<p><span>صفحه‌ای که دنبالش بودی، پیدا نشد.</span><br><span>مسیر درست را از بین گزینه‌های زیر پیدا کن!</span></p>')||!static404.includes('<strong>ارتباط و پشتیبانی</strong>'))throw new Error('Static 404 exact Persian copy is incomplete.');
...
for(const token of ['height:100dvh','max-height:100dvh','overflow:hidden','-webkit-tap-highlight-color:transparent','background:#DCF2ED','box-shadow:inset','id="neu-4-shadow"','id="donut-shadow"','id="donut-hole-mask"'])if(!static404.includes(token))throw new Error(`Static 404 contract missing: ${token}`);
const rendered404=renderNotFoundPage({brand:'Test'});
if(!rendered404.includes('<span>Test</span>')||!rendered404.includes('<span>عه</span><b>!</b> <span>اینجا کجاست؟</span>')||!rendered404.includes('fill="#D9ECE7"'))throw new Error('Fixed 404 renderer is incomplete.');
