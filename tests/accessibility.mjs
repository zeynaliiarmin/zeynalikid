...
for(const route of routes){
 const browser=await puppeteer.launch(launchOptions);const page=await browser.newPage();await page.setBypassServiceWorker(true);await page.setViewport({width:390,height:844});
 ...
 for(const violation of result.violations.filter(item=>item.impact==='critical'||item.impact==='serious'))blocking.push({route,id:violation.id,impact:violation.impact,nodes:violation.nodes.length,help:violation.help,targets:violation.nodes.slice(0,6).map(node=>({target:(node.target||[]).join(' '),html:String(node.html||'').slice(0,220)}))});
 await browser.close();
}
