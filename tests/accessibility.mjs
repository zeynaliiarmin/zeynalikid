...
const routes=['/','/courses','/products','/consultation','/child-info','/faq','/education','/about','/contact','/privacy','/track','/profile','/admin/login','/not-found-a11y'];
const blocking=[];
for(const route of routes){
 const browser=await puppeteer.launch(launchOptions);const page=await browser.newPage();await page.setBypassServiceWorker(true);await page.setViewport({width:390,height:844});await page.setRequestInterception(true);
 ...
 let result;for(let attempt=0;attempt<4;attempt++){try{await page.waitForFunction(()=>document.readyState==='complete'||document.readyState==='interactive',{timeout:5000});result=await new AxePuppeteer(page).withTags(['wcag2a','wcag2aa','wcag21a','wcag21aa']).analyze();break}catch(error){if(attempt===3)throw new Error(`${route}: ${String(error?.message||error)}`);await page.reload({waitUntil:'domcontentloaded',timeout:30000});await new Promise(resolve=>setTimeout(resolve,900));}}
