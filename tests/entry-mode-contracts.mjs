...
const [routes,defaults,support,header,app,entryMigration,pageChrome]=await Promise.all([
  read('src/config/routes.ts'),read('src/config/defaultSettings.ts'),read('src/app/appSupport.tsx'),read('src/components/Header.tsx'),read('src/App.tsx'),read('supabase/migrations/20260904113000_set_user_portal_entry_default.sql'),read('src/sections/PageChrome.tsx'),
]);
...
// the stage-3 refactor moved the header wiring into src/sections/PageChrome.tsx
needRe(pageChrome,/portalMode=\{\(cfg as any\)\?\.entryMode\s*!==\s*'track'\}/,'app does not pass the active entry preference to its header');
