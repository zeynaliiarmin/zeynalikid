...
for(const [mode,viewport] of [['mobile',{width:390,height:844}],['desktop',{width:1440,height:1000}]]){
 ...
 const result=await page.evaluate(mode=>{const rect=selector=>{const node=document.querySelector(selector);if(!node)return null;const value=node.getBoundingClientRect();return{x:value.x,y:value.y,width:value.width,height:value.height,right:value.right,bottom:value.bottom}};const quick=[...document.querySelectorAll('.zk-home-quick-item')].map(node=>{const r=node.getBoundingClientRect();return{x:r.x,y:r.y,width:r.width}});const container=rect('.zk-home-container');const core=document.querySelector('.zk-home-core-grid');const productRail=document.querySelector('.featured-products-rail');return{mode,v2:document.querySelector('.zk-home-page')?.classList.contains('zk-home-v2'),overflow:document.documentElement.scrollWidth-window.innerWidth,container,quick,core:core?{client:core.clientWidth,scroll:core.scrollWidth}:null,productRail:productRail?{client:productRail.clientWidth,scroll:productRail.scrollWidth,items:productRail.children.length}:null,services:rect('.zk-home-services'),coreSection:rect('.zk-home-core'),parents:rect('.zk-home-parents'),testimonials:rect('.zk-home-testimonials'),sections:document.querySelectorAll('.zk-home-section').length,h1:document.querySelectorAll('h1').length}},mode);
 if(!result.v2||result.overflow>1||!result.container||result.sections<8||!result.h1||!result.productRail||result.productRail.items<1)throw new Error(`${mode}: basic responsive shell failed ${JSON.stringify(result)}`);
 if(result.productRail.items>1&&result.productRail.scroll<=result.productRail.client)throw new Error(`${mode}: products are not horizontally scrollable ${JSON.stringify(result.productRail)}`);
 if(mode==='mobile'){
...
await browser.close();console.log('Responsive Home V2 passed on mobile and desktop.');
