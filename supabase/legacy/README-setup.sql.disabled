...
-- =========================================================
-- ۱۲) بخش C (مرحله ۶): جدول سوالات کاربران  
-- =========================================================
CREATE TABLE IF NOT EXISTS public.user_questions (  
  id BIGSERIAL PRIMARY KEY,  
  question TEXT NOT NULL,              -- متن سوال  
  question_en TEXT,                    -- ترجمه انگلیسی (اختیاری، توسط ادمین)  
  answer TEXT,                         -- پاسخ ادمین (null = پاسخ داده نشده)  
  answer_en TEXT,                      -- پاسخ انگلیسی (اختیاری)  
  voice_note_url TEXT,                 -- URL یادداشت صوتی (اختیاری)  
  page_source TEXT,                    -- از کدوم صفحه سوال رو پرسیده (faq/education/home)  
...
-- ایندکس برای فیلتر سریع بر اساس وضعیت  
CREATE INDEX IF NOT EXISTS idx_user_questions_status  
  ON public.user_questions(status);  
CREATE INDEX IF NOT EXISTS idx_user_questions_created_at  
  ON public.user_questions(created_at DESC);  

-- RLS: هر کسی می‌تونه سوال بفرسته، فقط خواندن عمومی  
ALTER TABLE public.user_questions ENABLE ROW LEVEL SECURITY;  
DROP POLICY IF EXISTS "public insert questions" ON public.user_questions;  
CREATE POLICY "public insert questions" ON public.user_questions  
  FOR INSERT WITH CHECK (true);  
DROP POLICY IF EXISTS "public read questions" ON public.user_questions;  
CREATE POLICY "public read questions" ON public.user_questions  
  FOR SELECT USING (true);  
DROP POLICY IF EXISTS "public update questions" ON public.user_questions;  
CREATE POLICY "public update questions" ON public.user_questions  
  FOR UPDATE USING (true);  
DROP POLICY IF EXISTS "public delete questions" ON public.user_questions;  
CREATE POLICY "public delete questions" ON public.user_questions  
  FOR DELETE USING (true);


-- =========================================================
-- ۱۳) بخش F2: جدول نظرات دوره‌ها (reviews)  
-- =========================================================
CREATE TABLE IF NOT EXISTS public.reviews (  
  id BIGSERIAL PRIMARY KEY,  
  course_id TEXT,                    -- شناسه دوره (اختیاری)  
  reviewer_name TEXT NOT NULL,       -- نام ناظر  
...
DROP POLICY IF EXISTS "public insert voice-notes" ON storage.objects;
CREATE POLICY "public insert voice-notes" ON storage.objects
...
CREATE POLICY "public update voice-notes" ON storage.objects
  FOR UPDATE USING (bucket_id = 'voice-notes');
