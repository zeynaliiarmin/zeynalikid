...
async function home(db:any,chatId:string,intro=true,messageId?:number){const [,settings]=await Promise.all([clearState(db,chatId),db.from('assistant_settings').select('enabled').eq('key','default').maybeSingle()]);const data=settings.data;await render(chatId,`${intro?`سلام؛ ربات خصوصی مدیریت دستیار ${BRAND} آماده است.\n\n`:''}وضعیت دستیار سایت: ${data?.enabled===true?'🟢 فعال':'🔴 غیرفعال'}\nیکی از گزینه‌های ساده زیر را انتخاب کنید:`,homeKeyboard,messageId)}

async function resolveId(db:any,prefix:string){const value=String(prefix||'').trim().toLowerCase();if(!/^[0-9a-f-]{4,36}$/.test(value))return null;const entries=Object.entries(tables),results=await Promise.all(entries.map(([,table])=>db.from(table).select('id').limit(1000))),found:any[]=[];for(let index=0;index<entries.length;index++){const [scope,table]=entries[index],{data}=results[index];for(const item of data||[])if(String(item.id).toLowerCase().startsWith(value))found.push({id:item.id,table,scope:scope as Scope})}return found.length===1?found[0]:null}
...
async function matchingRow(db:any,scope:Scope,question:string){const needle=normalizeAssistantText(question),{data,error}=await db.from(tables[scope]).select('*').limit(5000);if(error)throw error;return (data||[]).find((row:any)=>normalizeAssistantText(row.question)===needle)||null}
...
async function listKnowledge(db:any,chatId:string,scope:Selection,page=0,messageId?:number){
  ...
  const rows=scope==='both'?all.slice(page*size,page*size+size):all.slice(0,size),hasNext=scope==='both'?all.length>(page+1)*size:all.length>size;if(!rows.length){await render(chatId,'در این بخش دانشی وجود ندارد.',keyboard([[{text:'↩️ منوی اصلی',callback_data:'menu:home'}],backHome]),messageId);return}
...
async function pairedDraft(db:any,found:any,selection:Selection):Promise<Draft>{const row=found.row;if(selection!=='both')return {...row,scope:found.scope,source_scope:found.scope,edit_id:row.id,original_question:row.question,[`${found.scope}_id`]:row.id};const otherScope:Scope=found.scope==='public'?'admin':'public',other=await matchingRow(db,otherScope,row.question),publicRow=found.scope==='public'?row:other,adminRow=found.scope==='admin'?row:other,base=publicRow||adminRow;return {...base,scope:'both',source_scope:found.scope,edit_id:row.id,original_question:row.question,public_id:publicRow?.id,admin_id:adminRow?.id,target_tab:adminRow?.target_tab||'dashboard',target_focus:adminRow?.target_focus||'',action_label:adminRow?.action_label||'رفتن به بخش مرتبط',actions:publicRow?.actions||[],scope_answers_differ:Boolean(publicRow&&adminRow&&String(publicRow.answer)!==String(adminRow.answer))}}
async function showItem(db:any,chatId:string,prefix:string,selection:Selection='public',messageId?:number,resolved?:any){const found=resolved||await rowByPrefix(db,prefix);if(!found){await render(chatId,'دانش موردنظر پیدا نشد.',keyboard([backHome]),messageId);return}const row=found.row,draft=await pairedDraft(db,found,selection);await render(chatId,previewText(draft),keyboard([[{text:'ویرایش سؤال',callback_data:`item:editq:${short(row.id)}:${selection}`},{text:'ویرایش پاسخ',callback_data:`item:edita:${short(row.id)}:${selection}`}],[{text:publishedAndActive(row)?'پیش‌نویس کن':'منتشر و فعال کن',callback_data:`item:toggle:${short(row.id)}:${selection}`},{text:'حذف',callback_data:`item:delete:${short(row.id)}:${selection}`}],[{text:'↩️ بازگشت به فهرست',callback_data:`list:${selection}:0`},{text:'🏠 منوی اصلی',callback_data:'menu:home'}]]),messageId)}
async function matchingRows(db:any,found:any,selection:Selection){if(selection!=='both')return [found];const otherScope:Scope=found.scope==='public'?'admin':'public',other=await matchingRow(db,otherScope,found.row.question);return other?[found,{id:other.id,table:tables[otherScope],scope:otherScope,row:other}]:[found]}
...
async function handleCallback(db:any,chatId:string,data:string,messageId:number){
  ...
  if(data==='menu:quick'){await clearState(db,chatId);await setState(db,chatId,'quick_instruction',{scope:'public'});await render(chatId,'دستور آموزش را در یک پیام بنویسید؛ همان لحظه در اختیار همه دستیارهای سایت قرار می‌گیرد. نمونه:\n\nاگر کاربر گفت «هزینه مشاوره چقدر است؟» یا «قیمت مشاوره»، دقیقاً بگو «برای دیدن هزینه وارد فرم مشاوره شوید» و دکمه ثبت مشاوره را نمایش بده.',keyboard([[{text:'❌ لغو',callback_data:'menu:cancel'}]]),messageId);return}
  if(data==='menu:add'){await clearState(db,chatId);await setState(db,chatId,'mode',{scope:'public',actions:[]});await render(chatId,'ربات چطور پاسخ بدهد؟',modeKeyboard,messageId);return}
  ...
  if(data.startsWith('item:')){
    const [,action,prefix,value]=data.split(':'),selection:Selection=value==='both'?'both':value==='admin'?'admin':'public';if(action==='show'){await clearState(db,chatId);return showItem(db,chatId,prefix,selection,messageId)}const found=await rowByPrefix(db,prefix);if(!found){await render(chatId,'دانش پیدا نشد.',keyboard([backHome]),messageId);return}
    if(action==='toggle'){const active=publishedAndActive(found.row),changes=active?{status:'draft'}:{status:'published',is_active:true};await updateKnowledgeSelection(db,found,selection,changes);return showItem(db,chatId,prefix,selection,messageId,{...found,row:{...found.row,...changes}})}
...
async function handleText(db:any,chatId:string,incoming:string){
  ...
  if(command==='/start'||command==='/help')return home(db,chatId);if(command==='/cancel')return home(db,chatId,false);if(command==='/add'){await setState(db,chatId,'mode',{scope:'public',actions:[]});await send(chatId,'ربات چطور پاسخ بدهد؟',modeKeyboard);return}if(command==='/quick'){await clearState(db,chatId);await setState(db,chatId,'quick_instruction',{scope:'public'});await send(chatId,'دستور آموزش را در یک پیام بنویسید؛ همان لحظه در اختیار همه دستیارهای سایت قرار می‌گیرد.');return}if(command==='/list'){await clearState(db,chatId);return listKnowledge(db,chatId,'public',0)}if(command==='/test'){await clearState(db,chatId);await setState(db,chatId,'test_question',{scope:'public'});await send(chatId,'سؤال آزمایشی را بنویسید:');return}if(command==='/frequent')return frequentHome(chatId);if(command==='/unanswered')return showUnanswered(db,chatId);if(command==='/backup')return sendKnowledgeBackup(db,chatId);
...
serve(async req=>{
  ...
  const update=await req.json().catch(()=>({})),callback=update?.callback_query,message=callback?.message||update?.message,chatId=String(message?.chat?.id||''),messageId=Number(message?.message_id||0),owner=assistantTelegramOwner(),callbackAck=callback?.id?telegramAnswerCallback(String(callback.id)).catch(()=>null):Promise.resolve(null);
  if(!message||!chatId||chatId!==owner||message?.chat?.type!=='private'){await callbackAck;return Response.json({ok:true})}
  const db=getSupabaseAdmin();
  try{
    if(callback)await Promise.all([callbackAck,handleCallback(db,chatId,String(callback.data||''),messageId)]);
