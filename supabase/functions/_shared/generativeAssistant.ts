...
export async function generateGroundedAssistant(options:{question:unknown;knowledge:ScopedKnowledge[];mode:'public'|'admin';brand:string;language?:'fa'|'en';db?:any;image?:string|null}):Promise<GroundedAssistantResult>{
  ...
  const rules=[
    ...
    variationHint,
    options.language==='en'?'Answer in clear, natural English and keep the response under 180 words.':'پاسخ را با فارسی گفتاری مودبانه (آمیانهٔ آداب‌دار) و حداکثر ۱۸۰ کلمه بنویس — قواعد (الزامی): ۱) «می» و «نمی» را به فعل بچسبان: میرم، میگه، میخوام، نمیدونم؛ ۲) کلمات «ان» به «ون» بده: آسمون، میدون، دندون، خونه، بیمارستون، کتابخونه، کارخونه، جوون، مهمون، جهون، زمون، انسون، پنهون، نشون؛ ۳) «خوا» به «خا»: میخوام، میخوای، میخواد، میخواین؛ ۴) اعراب (کسره/فتحه/ضمه) ننویس: یه، اون، اینجوری، کدوم، واسه، چندتا، هیچوقت، هیچکدوم؛ ۵) «است» به «ـه» تغییر بده: خوبه، گرمه، مهمه؛ ۶) صور موجود گفتاری: میتونه، میدونه، میخوره، میبینه، میشنوه، آره، یعنی، چجوری، چقدر، کجا، واسه؛ ۷) کاملاً مؤدب بنویس: از الفاظ کوچه‌بازاری و بی‌ادبانه (مثل برار، چار، گنده، خطر رایج) هرگز استفاده نکن؛ داداش=برادر مجاز است ولی کلمات سنگین یا توهین‌آمیز ممنوع؛ ۸) لحن طبیعی و گرم: اول جمله کوتاه پاسخ، بعد ۲–۳ جملهٔ عملی؛ جمله‌سازی ساده و روان؛ از «باید/ندارد» رسمی‌نامه پرهیز کن و به‌جاش «باید» با کسره بگو: «بایِد» را به شکل «باید» بنویس بدون استرس. تا جای ممکن معادل فارسی واژه‌های انگلیسی را به کار ببر مگر نام رسمی یا واژهٔ تخصصی باشد.',
