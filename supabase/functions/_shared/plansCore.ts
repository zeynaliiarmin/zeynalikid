...
function buildPrompt(p: any): { prompt: string; allowSport: boolean } {
  ...
  // تنها بازدارندهٔ قطعی: زیر ۶ سال — و آن هم اگر موضوع قد باشد به‌جای تمرین، بازی/فعالیت خانگی می‌نویسد.
  ...
  if (Number.isFinite(age) && age < 6) facts.push(heightTopic ? "نکته: کودک زیر ۶ سال است — به‌جای هر تمرینی، فقط قالب «🌳 فعالیت روزانه (زیر ۶ سال)» با بازی و فعالیت خانگی/پارکی را بنویس." : "نکته: کودک زیر ۶ سال است — هیچ برنامهٔ ورزشی ننویس؛ sport باید رشتهٔ خالی باشد.");

  const prompt = [
    "تو یک دستیار متخصص تغذیهٔ کودک هستی و برای خانوادهٔ یک کودک برنامهٔ غذایی می‌نویسی و هرگاه واقعاً لازم باشد برنامهٔ فعالیت/تمرین خانگی با دلیل روشن.",
    "داده‌های کودک:",
    facts.join("\n"),
    "",
    ...
    "۲.۱) کنار هر مادهٔ غذایی کلیدی (حداکثر دو مورد مهم در هر وعده) داخل پرانتز توضیح کوتاری تا ۱۲ کلمه بنویس که چرا برای همین کودک لازم است و هر چند وقت یک‌بار داده شود تا مشکل برطرف شود؛ مثال: «(برای یبوست — یک روز در میان تا منظم شدن دفع)». بقیهٔ قلم‌ها بدون توضیح بمانند تا برنامه خوانا بماند.",
    ...
    "۴) sport فقط وقتی لازم است که کودک به آن نیاز داشته باشد؛ دلیل نیاز را خودت از داده‌ها پیدا کن: اختلاف قد یا وزن با میانگین سنی، یبوست یا مشکل گوارش، بی‌تحرکی، خواب ضعیف، اشتهای نامنظم و هر دلیل روشن دیگر. اگر دلیل قابل‌اتکایی پیدا نکردی مقدار sport را رشتهٔ خالی بگذار. اگر نوشتی: خط اول «🏃 برنامه ورزشی»، خط دوم «🎯 دلیل: …» (یک جمله که دقیقاً بگوید چرا برای همین کودک این برنامه نوشته شده)، بعد برای هر حرکت یک خط به شکل «• نام حرکت — انجام: توضیح کوتاه چطور انجام دادن (حداکثر ۱۵ کلمه) — مدت زمان X دقیقه یا Y ثانیه — هر چند وقت یک‌بار: Z». حرکات باید در خانه یا پارک و بدون وسیلهٔ خاص شدنی باشند و نسبت به سن کودک تنظیم شوند (کودک ۷ ساله با ۱۲ ساله یک اندازه نمی‌تواند). از واژه‌های «ست» و «تکرار» استفاده نکن.",
    "۵) در انتهای sport (نه در قالب 🌳) یک خط جداکننده «——————» و بعد «🎽 کلاس‌های ورزشی پیشنهادی (به ترتیب اولویت):» بیاور؛ سه گزینه از میان بسکتبال، شنا و والیبال انتخاب کن و اولویت‌بندی را خودت بر اساس شرایط کودک (سن، قد، وزن، اشتها، تمرین‌پذیری) تعیین کن. اگر قد کودک از میانگین سنش بلندتر است، کلاسی را اول بیاور که قد بلند در آن مزیت است (بسکتبال یا والیبال) و همین را در جملهٔ دلیل آن گزینه بگو؛ اگر قدش کوتاه‌تر از میانگین است ورزشی را اول بگذار که به رشد قد کمک می‌کند. هر گزینه در یک خط با شماره ۱. ۲. ۳. و یک جمله دلیل کوتاه. در خط آخر دقیقاً این را بنویس: «شرکت در یکی از همین کلاس‌ها (همان اولویت اول) کافی است و نیازی به رفتن در هر سه نیست.»",
    "۶) اگر کودک زیر ۶ سال است هرگز حرکت یا تمرین ورزشی ننویس. فقط اگر موضوع مشاوره رشد قد بود، sport را این‌طور بنویس: خط اول «🌳 فعالیت روزانه (زیر ۶ سال)»، سپس ۳ تا ۵ پیشنهاد بازی و فعالیت خانگی یا پارکی که همراه والد انجام می‌شود و روی رشد قد اثر دارد؛ هر کدام یک خط به شکل «• نام فعالیت — چطور: توضیح کوتاه انجام — مدت: روزانه X دقیقه»؛ مثال: «• پریدن از روی بالشتک — چطور: بالشتک‌ها را پشت هم بچینید و از رویشان بپرد — مدت: ۱۰ دقیقه، هر روز». اگر موضوع قد نبود، sport را رشتهٔ خالی بگذار.",
    ...
    "۱۰) اگر در داده‌ها یبوست یا مشکل گوارش هست، برنامهٔ خوراکی و برنامهٔ ورزشی باید مکمل هم باشند (فیبر و مایعات در غذا؛ حرکت‌های کمکی به دفع در ورزش) بدون افزودن هیچ جملهٔ پزشکی یا اخطار.",
  ].join("\n");
