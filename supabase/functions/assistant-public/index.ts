...
const supportResponse=(answer:string,phone:string,settings:any,question:string,code:string)=>({ok:true,answer:phone?`${answer}\nشماره تماس پشتیبانی: ${phone}`:answer,model:'internal-support',sources:[],actions:[{label:'ورود به صفحه ارتباط با ما',path:'/contact'}],suggestions:suggestionsFrom(settings,[],question),provider_called:false,blocked_admin:false,blocked_private:false,limit_code:code,support_phone:phone,remaining_daily:0});
...
const fixedResponse=(answer:string,suggestions:any[],actions:any[]=[],model='internal-context-policy')=>({ok:true,answer,model,sources:[],actions,suggestions,provider_called:false,blocked_admin:false,blocked_private:false,remaining_daily:20});
...
serve(async req=>{
  ...
  if(action==='generate'){
    const question=safe(body.question,500),language=responseLanguage(question,body.ui_language),clientId=String(body.client_id||'').trim();if(normalizeAssistantText(question).length<3||!/^[a-z0-9-]{16,80}$/i.test(clientId))return reply({error:'سؤال یا شناسه مرورگر معتبر نیست'},400,origin);
    ...
    const fixed=findKnowledgeRule(question,rows);
    ...
    if(!matches.length){await rememberUnanswered(db,question,safe(body.page_path,200));return deliver({ok:true,answer:localizedFallback,model:'internal-no-knowledge',sources:[],actions:[],suggestions:suggestionsFrom(settings,[],question),provider_called:false,blocked_admin:false,blocked_private:false,remaining_daily:20},200)}
    const [minute,daily]=await Promise.all([centralRateLimit(req,'assistant-public-mistral-minute',{maxRequests:5,windowMs:60_000},clientId),centralRateLimit(req,'assistant-public-mistral-day',{maxRequests:20,windowMs:86_400_000},clientId)]);
    if(!daily.ok)return deliver(supportResponse('لطفاً برای اطلاعات بیشتر با پشتیبانی تماس بگیرید.',supportPhone,settings,question,'daily_limit'),200);
    if(!minute.ok)return deliver({ok:true,answer:'ارتباط با دستیار موقتاً محدود شده؛ لطفاً ۳۰ ثانیه بعد دوباره تلاش کنید.',model:'internal-rate-limit',sources:[],actions:[],suggestions:suggestionsFrom(settings,[],question),provider_called:false,blocked_admin:false,blocked_private:false,limit_code:'minute_limit',support_phone:'',remaining_daily:daily.remaining},200);
    let result;
