...
serve(async req=>{
  ...
  try{
    ...
    if(action==='generate_admin'||action==='generate_preview'){
      const question=sanitizeAssistantQuestion(body.question);if(normalizeAssistantText(question).length<3)return jsonResponse({error:'سؤال مدیریتی معتبر نیست'},400,origin);if(customerIntent(question))return await searchCustomers(db,question,auth,origin);
      const [{data:adminKnowledge,error},{data:publicKnowledge}]=await Promise.all([db.from('assistant_admin_knowledge').select('id,question,answer,aliases,keywords,category,priority,target_tab,target_focus,action_label,response_mode,match_mode').eq('status','published').eq('is_active',true).order('priority',{ascending:false}).limit(500),db.from('assistant_knowledge').select('id,question,answer,aliases,keywords,category,priority,link_url,link_label,actions,response_mode,match_mode').eq('status','published').eq('is_active',true).order('priority',{ascending:false}).limit(500)]);if(error)throw error;
      const knowledge=[...(adminKnowledge||[]),...(publicKnowledge||[])] as ScopedKnowledge[],fixed=findKnowledgeRule(question,knowledge);let result:any,providerError='';
      if(fixed){const sources=adminSources([{item:fixed.item,score:fixed.score}] as any);result={answer:String(fixed.item.answer||''),model:fixed.item.response_mode==='refusal'?'internal-refusal-rule':'internal-exact-rule',sources,providerCalled:false}}
      else{const matches=relatedKnowledge(question,knowledge,6),fallbackSources=adminSources(matches);try{result=await generateGroundedAssistant({question,knowledge,mode:'admin',brand:BRAND})}catch(error){providerError=String((error as Error)?.message||'MISTRAL_PROVIDER');if(providerError==='MISTRAL_RATE_LIMIT')return jsonResponse({ok:true,answer:'سهمیه امروز دستیار تمام شده است.',model:'provider-limit',sources:[],actions:[],suggestions:[],customer_results:[],provider_called:false,provider_notice:'MISTRAL_RATE_LIMIT'},200,origin);result={answer:String(matches[0]?.item?.answer||''),model:'internal-fallback',sources:fallbackSources,providerCalled:false}}}
      const answer=result.answer||'در دانش تأییدشده عمومی و مدیریتی، اطلاعات کافی برای این سؤال ندارم.';await audit(db,auth,'assistant_admin_generate','assistant_knowledge','',{provider:result.providerCalled?'mistral':'internal',model:result.model,source_count:result.sources.length,error_code:providerError.slice(0,80)});
