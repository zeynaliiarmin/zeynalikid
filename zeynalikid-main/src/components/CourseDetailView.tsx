...
export default function CourseDetailView({ course, T, lang, onClose, onRegister, onConsult }: Props) {
  ...
  const reviews = [
    { name: isFa ? 'مادر علی' : 'Ali’s mom', rating: 5, text: isFa ? 'بسیار مفید و آرامش‌بخش بود.' : 'Very helpful and calming.' },
    { name: isFa ? 'مادر سارا' : 'Sara’s mom', rating: 4, text: isFa ? 'تغییرات خوبی در اشتها دیدیم.' : 'Saw good appetite changes.' },
  ];
    ...
    <div style={{ background: 'var(--zk-surface)', borderRadius: 22, overflow: 'hidden', border: '1px solid var(--zk-border)', boxShadow: 'var(--zk-shadow-medium)' }}>
      ...
      <div style={{ position: 'relative', height: 188, background: 'var(--zk-surface-muted)' }}>
        ...
        <div style={{ position: 'absolute', top: 12, left: 12, right: 12, display: 'flex', justifyContent: 'space-between' }}>
          ...
          {/* Heart / favorite button — Stage 6: local animated (outline → warm filled) */}
          <button 
            onClick={() => {
              const next = !isWish;
              setIsWish(next);
      ...
      <div style={{ padding: '16px 16px 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
          ...
          <button onClick={onRegister} style={{ background: 'var(--zk-primary)', color: '#fff', border: 0, padding: '11px 24px', borderRadius: 999, fontWeight: 700, fontSize: 14, minHeight: 48 }}>
            {isFa ? 'ثبت‌نام در دوره' : 'Enroll now'}
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 16px 20px' }}>
        ...
        {activeTab === 'reviews' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {reviews.map((r, i) => (
              <div key={i} style={{ background: 'var(--zk-surface-muted)', padding: 14, borderRadius: 14 }}>
