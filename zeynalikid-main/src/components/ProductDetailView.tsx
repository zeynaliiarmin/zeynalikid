...
export default function ProductDetailView({ product, T, lang, onClose, onAddToCart, onConsult }: Props) {
  ...
  const features = product.features?.length
    ? product.features
    : [
        isFa ? 'برنامه شخصی‌سازی‌شده بر اساس طبع کودک' : 'Personalized plan based on child’s temperament',
        isFa ? 'پشتیبانی ۷ روزه پس از خرید' : '7-day support after purchase',
    ...
    <div style={{ background: 'var(--zk-surface)', borderRadius: 22, overflow: 'hidden', border: '1px solid var(--zk-border)', boxShadow: 'var(--zk-shadow-medium)' }}>
      ...
      <div style={{ position: 'relative', height: 210, background: 'linear-gradient(145deg, #F8F4EF, #FDF8F3)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '18px' }}>
        ...
        <div style={{ position: 'absolute', top: 12, left: 12, right: 12, display: 'flex', justifyContent: 'space-between' }}>
          ...
          {/* Heart / favorite button — opposite side */}
          <button 
            style={{ 
              order: isFa ? 1 : 2,
              background: 'rgba(255,255,255,0.95)', 
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#111" strokeWidth="2.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '16px 16px 8px' }}>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
          <span style={{ fontSize: 10.5, padding: '2px 9px', borderRadius: 999, background: '#0EA5E9', color: '#fff', fontWeight: 700 }}>{product.category || (isFa ? 'برنامه شخصی‌سازی‌شده' : 'Personalized Plan')}</span>
        ...
        {activeTab === 'intro' && (
          <>
            ...
            <div style={{ marginTop: 18, padding: '14px 16px', background: 'var(--zk-surface-muted)', borderRadius: 16, fontSize: 13, lineHeight: 1.6 }}>
              {isFa ? 'این برنامه شخصی‌سازی‌شده با بررسی شرایط فرزند شما طراحی می‌شود و شامل راهنمایی‌های دقیق تغذیه‌ای و پیگیری است.' : 'This personalized plan is designed based on your child’s profile and includes detailed nutrition guidance and follow-up.'}
            </div>
            ...
            <div style={{ marginTop: 14, padding: '12px 15px', background: '#FEF3C7', border: '1px solid #F59E0B33', borderRadius: 14, fontSize: 12.5, lineHeight: 1.55, color: '#713F12' }}>
              {isFa ? 'نکته مهم: این محصول جایگزین مشاوره پزشکی یا درمان نیست. لطفاً تحت نظر متخصص از آن استفاده کنید.' : 'Important: This product is not a substitute for medical advice or treatment. Please use under professional guidance.'}
            </div>
          </>
        )}
        ...
        {activeTab === 'specs' && (
          <div style={{ display: 'grid', gap: 10, fontSize: 13.5 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 6, borderBottom: '1px solid var(--zk-border)' }}>
              <span>{isFa ? 'نوع محصول' : 'Type'}</span><span>{product.category || (isFa ? 'برنامه شخصی‌سازی‌شده' : 'Personalized Plan')}</span>
            </div>
