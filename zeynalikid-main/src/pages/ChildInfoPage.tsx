...
export default function ChildInfoPage({app}:{app:any}){
 ...
 const submit=async()=>{
  ...
  const e:any={...tongueErr}; if(!draft.gender)e.gender=lang==='en'?'Select gender':'جنسیت فرزند را انتخاب کنید'; const ag=+p2e(draft.age); if(!draft.age||isNaN(ag)||ag<2||ag>17)e.age=lang==='en'?'Age must be 2 to 17':'سن ۲ تا ۱۷ سال'; setErrs(e); if(Object.keys(e).length)return;
  let child_voice_note_url = '';
  if (voiceBlob && app.uploadVoiceNote) {
    try { const u = await app.uploadVoiceNote(voiceBlob); if (u) child_voice_note_url = u; } catch (e) { console.warn('voice upload fail', e); }
  }
 ...
 {fromConsultForm ? <>
  ...
  {cfg.formFields.notes?.show!==false&&<div style={{marginTop:12}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:7}}><label style={{fontSize:14,color:T.mut,fontWeight:700}}>{publicText('notes',cfg.formFields.notes.label)}</label><VoiceRecorder T={T} lang={lang} onRecorded={(blob:any)=>{setVoiceBlob(blob)}} onRemoved={()=>{setVoiceBlob(null)}}/></div><textarea style={S.ta} value={draft.notes||''} onChange={e=>setDraft({...draft,notes:e.target.value})} placeholder={trVal(cfg.formFields.notes.placeholder)}/></div>}
 </>}
