...
export default function CoursePaymentPage({app}:{app:any}){
 ...
 const submitPayment=()=>{const receiptText=receiptTextRef.current?.value||course.payment.receiptText||''; if(!course.payment.receipt&&!String(receiptText).trim()){setToast('لطفاً فیش واریزی را آپلود کنید یا متن پیامک را وارد کنید.');setTimeout(()=>setToast(''),3000);return}Promise.resolve(finalizeCourseRegistration({...course.payment,receiptText,receiptMethod:course.payment.receipt?'image':String(receiptText).trim()?'text':null})).catch((e:any)=>{console.error('finalize failed',e);setToast(lang==='en'?'An error occurred while submitting your information. Please contact support.':'خطایی در ثبت اطلاعات رخ داده است. لطفاً با پشتیبانی تماس بگیرید.');setTimeout(()=>setToast(''),6000)})};
 const formatCard=(v:any)=>String(v||'').replace(/\s+/g,'').replace(/(.{4})/g,'$1 ').trim(); const formatIban=(v:any)=>String(v||'').replace(/\s+/g,'').replace(/(.{2})/g,'$1 ').trim();
 ...
 function CopyMini({copied,onClick}:any){return <button onClick={onClick} title="کپی" style={{width:26,height:26,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',background:copied?'#16a34a':'rgba(255,255,255,.9)',color:copied?'#fff':'#111',fontSize:13,transition:'all .65s',flexShrink:0}}>{copied?'':'⧉'}</button>}
 ...
 return <div dir={lang==='en'?'ltr':'rtl'} style={{...S.page,minHeight:'100dvh',overflowX:'hidden',padding:'max(6px, env(safe-area-inset-top, 0px)) max(6px, env(safe-area-inset-right, 0px)) max(6px, env(safe-area-inset-bottom, 0px)) max(6px, env(safe-area-inset-left, 0px))',fontFamily:"'Vazirmatn','Tahoma',Arial,sans-serif"}}><Helmet><title>ثبت‌نام دوره | زینالیکید</title><meta name="description" content="تکمیل ثبت‌نام دوره تخصصی رشد و تغذیه کودکان و نوجوانان" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}{payCss}</style>{/* اصلاح ۷: افزایش padding کلی کارت و overflow:auto همیشگی تا با کارت‌های بانکی بزرگ‌تر، محتوا بدون بریدگی و با اسکرول داخلی متوازن نمایش داده شود */}<div style={{...S.card,maxWidth:390,height:'calc(100dvh - 12px)',overflowY:'auto',overflowX:'hidden',padding:10,borderRadius:18,display:'flex',flexDirection:'column',lineHeight:1.3,gap:2}}><Stepper step={4}/>
...
<div style={{background:T.card, border:`1px solid ${T.brd}`, borderRadius:16, padding:14, marginBottom:14, boxShadow:T.neuOut}}>
  ...
  <div style={{display:'flex', justifyContent:'space-between', fontWeight:800, fontSize:15}}>
    ...
    <span style={{color:T.acc}}>{course.selected?.price || '—'} {cfg.currencyUnit}</span>
  </div>
</div>
...
<div style={{marginTop:6,display:'flex',flexDirection:'column',gap:6}}>
  ...
  <div style={{border:`1.5px dashed ${T.brd}`,borderRadius:14,padding:12,background:T.inp,boxShadow:T.neuIn}}>
      ...
      <label style={{display:'inline-flex',alignItems:'center',gap:6,padding:'9px 14px',borderRadius:12,background:T.grad||T.acc||'#0F766E',color:'#fff',fontSize:12.5,fontWeight:800,cursor:'pointer',whiteSpace:'nowrap',boxShadow:'0 3px 10px rgba(15,118,110,0.25)'}}>
        <input
          ...
          onChange={async e=>{
            ...
            try{
              const url=await uploadReceiptWithProgress(f,course.payment.receipt,(p:number)=>setReceiptProgress(p));
              setCourse({...course,payment:{...course.payment,receipt:url,receiptText:'',receiptMethod:'image'}});
    ...
    <div style={{fontSize:11,color:course.payment.receipt?T.ok:T.mut,fontWeight:course.payment.receipt?800:500}}>
      {course.payment.receipt ? '✓ عکس فیش ثبت شده است' : publicText('noFileSelected','هیچ فایلی انتخاب نشده')}
    </div>
  ...
  {receiptProgress!==null&&(
    <div style={{marginTop:2}}>
      ...
      <div style={{fontSize:9.5,color:T.acc,marginTop:3,textAlign:'center',fontWeight:700}}>
    ...
    <div style={{display:'flex',alignItems:'center',gap:6,fontSize:11,color:T.ok,fontWeight:800}}>
      ...
      <button
        onClick={async()=>{
          ...
          setCourse({...course,payment:{...course.payment,receipt:'',receiptMethod:course.payment.receiptText?'text':null}});
        }}
  ...
  <textarea
    ref={receiptTextRef}
    rows={2}
    style={{...S.ta,minHeight:52,height:52,lineHeight:1.3,padding:6,resize:'none',opacity:course.payment.receipt?.length?0.58:1}}
    defaultValue={course.payment.receiptText||''}
    disabled={!!course.payment.receipt}
    onBlur={e=>setCourse({...course,payment:{...course.payment,receiptText:e.target.value,receiptMethod:e.target.value.trim()?'text':course.payment.receipt?'image':null}})}
    placeholder={publicText('receiptSmsPlaceholder','متن پیامک واریزی را اینجا وارد کنید...')}
  />

  {course.payment.receiptText?.trim()&&(
