...
export default function AdminLoginPage({app}:{app:any}){
 ...
 const ok=async()=>{
   ...
   if(normal){try{await loginAdminSession(enteredPhone||String(adminPhone),enteredPassword);setAErr('');if(biometricSupported()&&!hasAdminBiometric()&&confirm('آیا ورود با اثر انگشت یا Face ID را روی این دستگاه فعال می‌کنید؟')){try{await enrollAdminBiometric(String(adminPhone))}catch{}}done();}catch(e:any){setAErr(e?.message||'اتصال امن به سرور انجام نشد');}}else setAErr('شماره تماس یا رمز عبور اشتباه است');
 };
