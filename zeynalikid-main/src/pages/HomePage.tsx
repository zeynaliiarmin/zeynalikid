...
export default function HomePage({app}:{app:any}){
 ...
 return <main className="zk-home-page" dir={isRtl?'rtl':'ltr'} style={{...app.S?.page,paddingBottom:92,flexDirection:'column',alignItems:'center',background:T.bg,color:T.txt,overflowX:'hidden'}}>
  ...
  <div className="zk-home-container" style={{paddingTop:8}}>
   ...
   {trustBoxImage.enabled!==false&&cfg.trustMessages?.health?.length>0&&<section className="zk-home-section" style={{marginTop:0}}><TrustBoxWithImage text={cfg.trustMessages.health[0]?.title||''} imageUrl={'/images/asset13c-trust-parent-care.webp'} imageAlt={trustBoxImage.alt||'مادر و کودک'} imagePosition={isRtl?'right':'left'} T={T}/></section>}
...
{askOpen&&<AskQuestionForm T={T} lang={lang} pageSource="home" onClose={()=>setAskOpen(false)} onSubmit={async(q,v)=>{await submitUserQuestion(q,v,'home');}}/>}</section>;
}
