 ...
 * ─ Mobile/tablet (<1024px): sidebar becomes an 85vw Drawer sliding from the
        ...
        {groups.map(g => {
          ...
          return (
            <React.Fragment key={g.id}>
              <button
                type="button"
                className={`zkad-nav-item ${isGroupActive ? 'zkad-active' : ''}`}
                aria-current={isGroupActive ? 'page' : undefined}
                aria-expanded={hasSub ? isExpanded : undefined}
                onClick={() => {
                  if (hasSub) setExpanded(prev => { const n = new Set(prev); if (n.has(g.id) && !isGroupActive && !(g.items || []).some(i => i.id === active)) n.delete(g.id); else n.add(g.id); return n; });
                  go(g.id);
                }}
              >
                <span className="zkad-nav-ic">{g.icon}</span>
                <span className="zkad-nav-lbl">{g.label}</span>
                {hasSub && <span className={`zkad-caret ${isExpanded ? 'zkad-open-c' : ''}`}><ZkChevronDownIcon size={15} /></span>}
              </button>
              {hasSub && isExpanded && (
