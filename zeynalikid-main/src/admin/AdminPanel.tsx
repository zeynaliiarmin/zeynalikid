...
const Field=useCallback(({label,value,onChange,ph,type='text',required=false}:any)=>{
 ...
 const navGroups:AdminNavGroup[]=[
  ...
  const ChipGroup=useMemo(()=>({label,options,val,set}:{label:string,options:string[],val:string,set:(v:string)=>void})=><div className="zkad-chipgroup"><span className="zkad-chipgroup-lbl">{label}</span><div className="zkad-chiprow">{options.map(o=><button type="button" key={o} className={`zkad-chip ${val===o?'on':''}`} onClick={()=>set(o)}>{o}</button>)}</div></div>,[]);
...
<div style={{ position: 'fixed', bottom: 0, right: 0, pointerEvents: 'none', zIndex: 5000 }}><div style={{ pointerEvents: 'auto' }}><AdminSpeedDialFAB T={T} lang={lang} onNavigate={(id:string)=>setATab(id)} onSave={()=>setSave(editCfg)} /></div></div></AdminLayout></div>}
 ...
 const [subTab,setSubTab]=useState<'parent'|'course'|'manage'|'corrective'>('parent');
...
${JSON.stringify(h.data,null,2)}`</pre>)}</details>}
   ...
   {subTab==='manage'&&<div>
    ...
    {/* فیلد وضعیت مشاوره */}
    {(sub.type==='consultation'||activeConsultRecord.type==='consultation')&&<div style={{marginTop:10}}><label style={S.lbl}>وضعیت مشاوره</label><select style={S.inp} value={sub.consultationStatus||activeConsultRecord.consultationStatus||'مشاوره اولیه'} onChange={e=>{const val=e.target.value; setSubs((list:any[])=>list.map(x=>x.id===sub.id||x.id===activeConsultRecord.id?{...x,consultationStatus:val,consultationStatusChangedAt:new Date().toISOString(),changeHistory:logChange(x,`تغییر وضعیت مشاوره به ${val}`)}:x))}}>{['مشاوره اولیه','مشاوره شده','اصلاحی','ثبتی','پیگیری','پیگیری آخر ماه'].map(x=><option key={x}>{x}</option>)}</select></div>}
     ...
     {filtered.map((it:any,localIdx:number)=>{
      ...
      return <div key={it.id||gi} style={{border:`1px solid ${T.brd}`,borderRadius:12,padding:10,marginBottom:8,marginTop:10}}>
       ...
       {type==='image'&&<><label style={{...S.lbl,marginTop:4}}>لینک تصویر خارجی (VPN روشن)</label>
        <input dir="ltr" style={{...S.inp,marginBottom:6}} defaultValue={it.platforms?.externalImage||''} onBlur={e=>{const v=e.target.value.trim(); const a=document.activeElement as HTMLElement|null; setTimeout(()=>chgPlatform(gi,'externalImage',v),0); if(a && a!==e.target) requestAnimationFrame(()=>{try{(a as HTMLElement).focus({preventScroll:true} as any)}catch{}})}} placeholder="https://..."/>
        <label style={S.lbl}>لینک تصویر داخلی (VPN خاموش)</label>
        <input dir="ltr" style={{...S.inp,marginBottom:6}} defaultValue={it.platforms?.internalImage||''} onBlur={e=>{const v=e.target.value.trim(); const a=document.activeElement as HTMLElement|null; setTimeout(()=>chgPlatform(gi,'internalImage',v),0); if(a && a!==e.target) requestAnimationFrame(()=>{try{(a as HTMLElement).focus({preventScroll:true} as any)}catch{}})}} placeholder="https://..."/></>}
       {type==='audio'&&<><label style={{...S.lbl,marginTop:4}}>لینک صوتی خارجی (VPN روشن)</label>
        <input dir="ltr" style={{...S.inp,marginBottom:6}} defaultValue={it.platforms?.externalAudio||''} onBlur={e=>{const v=e.target.value.trim(); const a=document.activeElement as HTMLElement|null; setTimeout(()=>chgPlatform(gi,'externalAudio',v),0); if(a && a!==e.target) requestAnimationFrame(()=>{try{(a as HTMLElement).focus({preventScroll:true} as any)}catch{}})}} placeholder="https://..."/>
        <label style={S.lbl}>لینک صوتی داخلی (VPN خاموش)</label>
        <input dir="ltr" style={{...S.inp,marginBottom:6}} defaultValue={it.platforms?.internalAudio||''} onBlur={e=>{const v=e.target.value.trim(); const a=document.activeElement as HTMLElement|null; setTimeout(()=>chgPlatform(gi,'internalAudio',v),0); if(a && a!==e.target) requestAnimationFrame(()=>{try{(a as HTMLElement).focus({preventScroll:true} as any)}catch{}})}} placeholder="https://..."/></>}
       {/* پلتفرم‌های سفارشی */}
