...
const Field=useMemo(()=>memo(function MemoField({label,value,onChange,ph,type='text',required=false}:any){const [local,setLocal]=useState(value??'');const inputRef=useRef<HTMLInputElement|null>(null);const isNumeric=/phone|whatsapp|شماره|کارت|شبا|قیمت|price|کد|postal|zip|سن|قد|وزن|age|height|weight/i.test(String(label||''));useEffect(()=>setLocal(value??''),[value]);const handle=useCallback((e:any)=>setLocal(isNumeric?p2e(e.target.value):e.target.value),[isNumeric]);const commit=useCallback(()=>onChange(isNumeric?p2e(local):local),[onChange,local,isNumeric]);return <div style={{marginBottom:13}}><label style={S.lbl}>{label}{required&&<span style={{color:'#F59E0B',marginInlineStart:4,fontWeight:800}}>*</span>}</label><input ref={inputRef} inputMode={isNumeric?'numeric':undefined} type={type} style={S.inp} value={local} onChange={handle} onBlur={commit} onFocus={scrollFocus} placeholder={ph}/></div>}),[S,T.err,scrollFocus]);
 ...
 const navGroups:AdminNavGroup[]=[
  ...
  function StatCard({icon,label,value,sub,delta,dot=false,tone='info',active=false,onClick}:{icon:React.ReactNode,label:string,value:any,sub?:string,delta?:number,dot?:boolean,tone?:string,active?:boolean,onClick?:()=>void}){return <div role={onClick?"button":undefined} tabIndex={onClick?0:undefined} onClick={onClick} onKeyDown={onClick? (e:any)=>{if(e.key==='Enter'||e.key===' ') {e.preventDefault(); onClick();}}:undefined} className={`zkad-stat-card ${tone==='warn'?'zkad-warn-card':''} ${active?'zkad-stat-active':''} ${onClick?'zkad-clickable':''}`} style={onClick?{cursor:'pointer',outline: active?`2px solid ${'var(--zkad-acc)'}`:undefined, borderColor: active? 'var(--zkad-acc)':undefined}:undefined}>
...
<div style={{ position: 'fixed', bottom: 0, right: 0, pointerEvents: 'none', zIndex: 5000 }}><div style={{ pointerEvents: 'auto' }}><AdminSpeedDialFAB T={T} lang={lang} onNavigate={(id:string)=>setATab(id)} onSave={()=>setSave(editCfg)} /></div></div></AdminLayout></div>}
 ...
 // تب‌های پویا — فقط تب‌هایی که داده دارند نمایش داده می‌شوند
 const hasParent = !!(sub.pName || sub.fullPhone || sub.age || sub.gender || sub.height || sub.weight || (sub.topics||[]).length || sub.digest || sub.appetite || sub.disease || (sub.specials||[]).length || sub.notes || (sub.tonguePhotos||[]).length);
 const hasCourse = !!(sub.course || sub.shipping);
 const hasPayment = !!(sub.payment || sub.course);
 const hasCorrective = !!(sub.correctiveData || sub.corrective || sub.showCorrectiveTab);
 const tabsRaw:[('parent'|'course'|'payment'|'manage'|'corrective'),string][]=[['parent','۱. والد و فرزند'],['course','۲. دوره و ارسال'],['payment','۳. پرداخت'],['manage','۴. مدیریت و پیگیری'],['corrective','۵. اصلاحی']];
 const tabs = tabsRaw.filter(([id])=> {
   if(id==='parent') return hasParent;
   if(id==='course') return hasCourse;
   if(id==='payment') return hasPayment;
   if(id==='corrective') return hasCorrective;
   return true;
 });
  useEffect(()=>{ if(!tabs.some(([id])=> id===subTab) && tabs.length>0) setSubTab(tabs[0][0] as any); }, [tabs.map(t=>t[0]).join(','), subTab]);
 return <div className={`zkad-card ${sub.unread||sub.isNew?'zkad-unread':''}`} style={{marginBottom:10,borderColor:sub.priority==='high'?'var(--zkad-err)':sub.unread?'var(--zkad-acc)':undefined}}>
  ...
  {open&&<div style={{padding:13,borderTop:`1px solid ${T.brd}`,fontSize:12,lineHeight:2,background:T.card}}>
   {/* نوار تب‌های پویا — فقط تب‌های دارای داده */}
   <div style={{display:'grid',gridTemplateColumns:`repeat(${tabs.length},1fr)`,gap:6,marginBottom:12}}>{tabs.map(([id,label])=><button key={id} onClick={()=>setSubTab(id)} style={{padding:'8px 4px',borderRadius:11,border:'none',background:subTab===id?T.soft:T.card,color:subTab===id?T.acc:T.mut,cursor:'pointer',fontFamily:'inherit',fontSize:11,fontWeight:700,boxShadow:subTab===id?T.neuIn:T.neuOut}}>{label}</button>)}</div>
