...
import { ZkArrowUpIcon, ZkArrowDownIcon, ZkChevronUpIcon, ZkChevronDownIcon, ZkCheckIcon, ZkCloseIcon,
 ...
 ZkDownloadIcon, ZkFilterIcon, ZkPhoneIcon, ZkSendIcon, ZkClockIcon, ZkWarnIcon, ZkPlusIcon, ZkUploadIcon } from './adminIcons';
...
const Field=useMemo(()=>memo(function MemoField({label,value,onChange,ph,type='text',required=false}:any){const [local,setLocal]=useState(value??'');const inputRef=useRef<HTMLInputElement|null>(null);const isNumeric=/phone|whatsapp|شماره|کارت|شبا|قیمت|price|کد|postal|zip|سن|قد|وزن|age|height|weight/i.test(String(label||''));useEffect(()=>setLocal(value??''),[value]);const handle=useCallback((e:any)=>setLocal(isNumeric?p2e(e.target.value):e.target.value),[isNumeric]);const commit=useCallback(()=>onChange(isNumeric?p2e(local):local),[onChange,local,isNumeric]);return <div style={{marginBottom:13}}><label style={S.lbl}>{label}{required&&<span style={{color:'#F59E0B',marginInlineStart:4,fontWeight:800}}>*</span>}</label><input ref={inputRef} inputMode={isNumeric?'numeric':undefined} type={type} style={S.inp} value={local} onChange={handle} onBlur={commit} onFocus={scrollFocus} placeholder={ph}/></div>}),[S,T.err,scrollFocus]);
 ...
 function Admin(){
  ...
  const payOf=(x:any)=>x.payment?.receipt?'پرداخت‌شده':x.course?'در انتظار پرداخت':'بدون پرداخت';
 ...
 const navGroups:AdminNavGroup[]=[
  ...
  // Stage 7B: کارت KPI متراکم و گرم — عدد از داده واقعی، آیکن گوشه بالا، شاخص تغییر، نقطه هشدار فقط وقتی مقدار > ۰
  function StatCard({icon,label,value,sub,delta,dot=false,tone='info'}:{icon:React.ReactNode,label:string,value:any,sub?:string,delta?:number,dot?:boolean,tone?:string}){return <div className={`zkad-stat-card ${tone==='warn'?'zkad-warn-card':''}`}>
  ...
  function ChipGroup({label,options,val,set}:{label:string,options:string[],val:string,set:(v:string)=>void}){return <div className="zkad-chipgroup"><span className="zkad-chipgroup-lbl">{label}</span><div className="zkad-chiprow">{options.map(o=><button type="button" key={o} className={`zkad-chip ${val===o?'on':''}`} onClick={()=>set(o)}>{o}</button>)}</div></div>}
...
<div className="zkad-stat-grid">
 <StatCard tone="info" icon={<ZkReviewsIcon size={18}/>} label="کل فرم‌های مشاوره" value={faNum(consultSubs.length)} delta={consultWeek-consultPrev} sub={`این هفته: ${faNum(consultWeek)}`}/>
 <StatCard tone="ok" icon={<ZkCalendarIcon size={18}/>} label="فرم‌های امروز" value={faNum(todaySubs.length)} sub={`هفته جاری: ${faNum(weekAll)}`}/>
 <StatCard tone="info" icon={<ZkOrdersIcon size={18}/>} label="ثبت‌نام دوره موفق" value={faNum(paidCourseSubs.length)} sub={`از مجموع ${faNum(courseSubs.length)}`}/>
 <StatCard tone="warn" icon={<ZkClockIcon size={18}/>} label="در انتظار بررسی" value={faNum(pendingList.length)} dot={pendingList.length>0} sub="فرم‌ها و سفارش‌های بررسی‌نشده"/>
</div>
 ...
 return <div className={`zkad-card ${sub.unread||sub.isNew?'zkad-unread':''}`} style={{marginBottom:10,borderColor:sub.priority==='high'?'var(--zkad-err)':sub.unread?'var(--zkad-acc)':undefined}}>
  ...
  {open&&<div style={{padding:13,borderTop:`1px solid ${T.brd}`,fontSize:12,lineHeight:2,background:T.card}}>
   ...
   {subTab==='parent'&&<div>
    ...
    {(sub.tonguePhotos||[]).length>0&&<div style={{marginTop:10}}>
     <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:6}}>
      <b><ZkCameraIcon size={14} color={T.acc}/> عکس‌های زبان ({sub.tonguePhotos.length})</b>
   ...
   {subTab==='payment'&&<div>
    ...
    {sub.payment?.receipt&&<div style={{display:'flex',alignItems:'center',gap:8,marginTop:8}}><span style={{color:T.ok,fontSize:12}}><ZkCheckCircleIcon size={13}/> فیش آپلود شده</span><button onClick={async()=>{if(!confirm('آیا از حذف کامل این فیش واریزی مطمئن هستید؟ این عملیات قابل بازگشت نیست.'))return; try{await deleteStoredImage(sub.payment.receipt); const updatedPayment={...(sub.payment||{}),receipt:'',receipt_image:'',receiptDeletedAt:new Date().toISOString()}; setSubs((list:any[])=>list.map(x=>x.id===sub.id?{...x,payment:updatedPayment,changeHistory:logChange(x,'حذف دستی فیش واریزی')}:x)); alert('فیش با موفقیت حذف شد.')}catch(e){alert('خطا در حذف فیش. لطفاً مجدداً تلاش کنید.')}}} style={{padding:'4px 10px',borderRadius:8,border:`1px solid ${T.err}`,background:`${T.err}10`,color:T.err,cursor:'pointer',fontSize:11,fontFamily:'inherit'}}><ZkTrashIcon size={13}/> حذف فیش</button></div>}
    {sub.payment?.receiptDeletedAt&&!sub.payment?.receipt&&<div style={{fontSize:11,color:T.mut,marginTop:6}}>فیش حذف شده: {new Date(sub.payment.receiptDeletedAt).toLocaleDateString('fa-IR')}</div>}
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:10}}><div><label style={S.lbl}><ZkMoneyIcon size={13}/> قیمت پرداختی (تومان)</label><input inputMode="numeric" style={S.inp} defaultValue={sub.payment?.amount||''} onBlur={e=>{const val=parseInt(p2e(e.target.value).replace(/\D/g,''));const cur=sub.payment?.amount; if(!isNaN(val)&&val!==cur){const updatedPayment={...(sub.payment||{}),amount:val}; setSubs((list:any[])=>list.map(x=>x.id===sub.id?{...x,payment:updatedPayment,changeHistory:logChange(x,`ثبت قیمت پرداختی: ${val.toLocaleString()} تومان`)}:x))}}} placeholder="مثلاً 250000"/></div><div><label style={S.lbl}><ZkCalendarIcon size={13}/> تاریخ پرداخت</label><input style={S.inp} defaultValue={sub.payment?.paidAt||''} onBlur={e=>{const val=p2e(e.target.value).trim(); if(val&&val!==sub.payment?.paidAt){const updatedPayment={...(sub.payment||{}),paidAt:val}; setSubs((list:any[])=>list.map(x=>x.id===sub.id?{...x,payment:updatedPayment,changeHistory:logChange(x,`ثبت تاریخ پرداخت: ${val}`)}:x))}}} placeholder="1403/05/20"/></div></div>
   </div>}
   ...
   {subTab==='manage'&&<div>
    ...
    <div style={{marginTop:10}}><b>پیگیری‌ها:</b><div style={{display:'flex',gap:7,marginTop:6}}>{[0,1,2,3,4].map(i=><button key={i} className="zkad-fu-btn" title={`پیگیری مرحله ${i+1}`} onClick={()=>fu(i)} style={{width:34,height:34,borderRadius:'50%',border:`2px solid ${(sub.followUps||[])[i]==='done'?T.ok:(sub.followUps||[])[i]==='miss'?T.err:T.brd}`,background:'transparent',color:(sub.followUps||[])[i]==='done'?T.ok:(sub.followUps||[])[i]==='miss'?T.err:T.mut,cursor:'pointer'}}>{(sub.followUps||[])[i]==='done'?<ZkCheckIcon size={13}/>:(sub.followUps||[])[i]==='miss'?<ZkCloseIcon size={13}/>:''}</button>)}</div></div>
...
{JSON.stringify(h.data,null,2)}</pre>)}</details>}</div>}
 ...
 function Box({title,children}:any){return <section className="zkad-panel-card" style={{marginBottom:12}}><h3 style={{fontSize:13.5,color:T.ttl,margin:'0 0 12px',fontWeight:800,lineHeight:1.6,display:'flex',alignItems:'center',gap:7}}>{title}</h3>{children}</section>}
 ...
 function CountryEditor(){return <Box title="مدیریت کدهای کشور">{editCfg.countryCodes.map((c:any,i:number)=><div key={c.id} style={{display:'grid',gridTemplateColumns:'55px 1fr 80px 1.4fr 45px',gap:6,marginBottom:6}}><input style={S.inp} defaultValue={c.flag} onBlur={e=>chgCountry(i,'flag',e.target.value)}/><input style={S.inp} defaultValue={c.name} onBlur={e=>chgCountry(i,'name',e.target.value)}/><input style={S.inp} defaultValue={c.code} onBlur={e=>chgCountry(i,'code',p2e(e.target.value))}/><input style={S.inp} defaultValue={c.regex} onBlur={e=>chgCountry(i,'regex',e.target.value)}/><button className="zkad-iconbtn t-err" title="حذف کشور" disabled={c.locked} onClick={()=>setEditCfg({...editCfg,countryCodes:editCfg.countryCodes.filter((_:any,j:number)=>j!==i)})}><ZkCloseIcon size={13}/></button></div>)}<button style={AdminBtn()} onClick={()=>setEditCfg({...editCfg,countryCodes:[...editCfg.countryCodes,{id:'c'+uid(),name:'کشور جدید',code:'+0',flag:'',regex:'^\\d{7,}$'}]})}><ZkPlusIcon size={13}/> افزودن کشور</button></Box>}
