...
function flagToEmoji(code: string): string {  
  const c = String(code || '').trim().toUpperCase();  
  if (c.length !== 2 || !/^[A-Z]{2}$/.test(c)) return code || '';  
  return String.fromCodePoint(  
    127397 + c.charCodeAt(0),  
    127397 + c.charCodeAt(1)  
  );  
}
function labelCountry(c:any,lang:any){return `${flagToEmoji(c.flag)} ${lang==='en'?(c.nameEn||c.name):c.name} ${c.code}`}
function shortCountry(c:any){return `${flagToEmoji(c.flag)} ${c.code}`}
