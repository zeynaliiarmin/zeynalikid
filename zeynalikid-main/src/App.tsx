...
import { isSupabaseConfigured, supabase, fetchSettings, createSubmission, saveSettings as saveSettingsRemote, trackPageView, checkDuplicatePhone } from './lib/supabase';
...
function Footer({cfg,T,lang,setView}:{cfg:any,T:any,lang:Lang,setView:(v:string)=>void}){
  ...
  const socialLinks = [
    ...
    { key: 'telegram', label: 'Telegram', url: c.telegram ? `https://t.me/${String(c.telegram).replace('@','')}` : null, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 4 3 11l6 2 2 6 10-15z"/></svg> },
...
function App(){
 ...
 async function finalizeCourseRegistration(paymentOverride?:any){const pay=paymentOverride||course.payment; const fp=fullPhone(course.form.phoneCc,course.form.phone); const data={...fd,pName:course.form.receiver||fd.pName,cc:course.form.phoneCc,pPhone:course.form.phone,fullPhone:fp}; let trackingCode=''; let existingCodes:string[]=[]; let existingList:any[]=[]; try{let list:any[]=[]; if(isSupabaseConfigured){try{list=await checkDuplicatePhone(fp)?getLS(SK.subs,[]):getLS(SK.subs,[])}catch{list=getLS(SK.subs,[])}}else{list=getLS(SK.subs,[])} existingList=list; existingCodes=list.map((x:any)=>String(x.trackingCode||'')).filter(Boolean); const prevSame=list.find((x:any)=>digits(x.fullPhone||'')===digits(fp)&&x.trackingCode); if(prevSame)trackingCode=prevSame.trackingCode}catch{} if(!trackingCode)trackingCode=generateUniqueTrackingCode(existingCodes,cfg.trackingDigitCount||5);
