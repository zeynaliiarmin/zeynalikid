    ...
    <footer style={{marginTop:32,background:`linear-gradient(180deg, ${T.soft} 0%, ${T.bg} 100%)`,borderTop:`1px solid ${T.brd}`,color:T.txt,fontFamily:'var(--zk-font)',position:'relative',zIndex:1,paddingBottom:'calc(12px + env(safe-area-inset-bottom, 0px))'}}>
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'28px 16px 18px'}}>
        ...
        <div>
          ...
          <div style={{display:'grid',gap:7}}>{navLinks.map(l=><button key={l.key} onClick={()=>setView(l.view)} style={{textAlign:'right',background:'transparent',border:0,color:T.txt,fontSize:14,fontWeight:600,padding:'5px 0',minHeight:40,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><span style={{color:T.acc}}>→</span> {l.label}</button>)}</div>
        </div>
        <div>
