...
function App(){
 ...
 // مشاور ارجاع‌دهنده از URL (?ad=CODE)
 const [referralConsultant,setReferralConsultant]=useState<any|null>(null);
 useEffect(()=>{ const code=getReferralCodeFromUrl(); if(code){ const c=findConsultantByCode((cfg as any).consultants, code); if(c){ setReferralConsultant(c); try{ sessionStorage.setItem('zk_referral_code',code); }catch{} } } },[]);
