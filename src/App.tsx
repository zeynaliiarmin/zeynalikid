...
function App(){
 ...
 function chooseDest(dest:string,cr:any){const methods=cfg.shippingMethods[dest].filter((m:any)=>m.active).sort((a:any,b:any)=>(a.order||0)-(b.order||0)); const def=methods.find((m:any)=>m.default)||methods[0]; setCourse((c:any)=>({...c,selected:cr,dest,shippingMethod:def?.id||'',form:{...c.form,country:dest==='iran'?(lang==='en'?'Iran':'ایران'):'',receiver:fd.pName,phoneCc:fd.cc,phone:fd.pPhone}})); setShipModal(null); const hasChild=!!(fd.age&&fd.gender); setView(hasChild?'course-shipping':'child-info'); {const dl=Date.now()+COURSE_TIMER_MS;setFlowDeadline(dl);try{sessionStorage.setItem('zkid_flow_deadline',String(dl));}catch{}} flowExpiredRef.current=false;}
 ...
 const markIncomplete=(reasonEn:string,reasonFa:string)=>{
  ...
  try{
   ...
   if(hasData){
    ...
    const entry:any={id:uid(),trackingCode,type:'course',date:today(),time:now(),...data,category:'ثبتی ناقص',consultationStatus:'ناقص',orderStatus:'ناقص',incomplete:true,priority:'normal',unread:true,isNew:true,followReminder:false,followUps:[null,null,null,null,null],adminNotes:lang==='en'?reasonEn:reasonFa,usageInstructions:'',timeSlot:'',course:course.selected,shipping:{dest:course.dest,method:course.shippingMethod,...course.form,estimatedDelivery:deliveryText(),optionalSendDate:course.optionalSendDate},payment:{...course.payment},childInfo:course.childInfo||null,tonguePhotos:course.tonguePhotos||[],editHistory:course.editedHistory||[],advisor:null};
...
})()}<ErrorAlertHost cfg={cfg} lang={lang} /><div style={{paddingTop:showHeader?72:0,position:'relative',zIndex:1}}>{(flowDeadline&&timerViews.includes(view))&&<div style={{maxWidth:600,margin:'0 auto',padding:showHeader?'10px 14px 2px':'calc(14px + env(safe-area-inset-top, 0px)) 14px 2px',position:'relative',zIndex:5}}><CourseTimer deadline={flowDeadline} lang={lang}/></div>}{page}</div></>;
}
