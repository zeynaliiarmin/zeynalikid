...
function App(){
 ...
 const initialLang:Lang=(()=>{
  try{
    ...
    if(ql==='fa'||ql==='en')return ql as Lang;
  }catch{}
  return getLS('zkid_lang','fa') as Lang;
})();
const [lang,setLang]=useState<Lang>(initialLang);
// On mount: if URL has ?lang=, persist it to LS so future visits stay on that language.
useEffect(()=>{
  try{
    const usp=new URLSearchParams(window.location.search);
    const ql=usp.get('lang');
    if((ql==='fa'||ql==='en')&&getLS('zkid_lang','fa')!==ql){setLS('zkid_lang',ql);}
  }catch{}
...
const page=<AppRoutes app={app} ui={uiValue} flow={flowValue} admin={adminValue} adminAuthed={adminAuthed} referralReady={referralReady} referralConsultant={referralConsultant}/>;
 ...
 // URL param just gives crawlers a crawlable language URL).
 const buildLangHref=(l:string)=>{
   try{
     const u=new URL(canonicalUrl);
     u.searchParams.set('lang',l);
     return u.toString();
   }catch{return `${canonicalUrl}?lang=${l}`;}
 };
...
@media (min-width: 900px){
  ...
  .zk-edu-grid{grid-template-columns:repeat(2,minmax(0,1fr)) !important;}
}
@media (min-width: 1280px){
  .zk-card{max-width:880px !important;}
  .zk-edu-grid{grid-template-columns:repeat(3,minmax(0,1fr)) !important;}
}
@media (prefers-reduced-motion: reduce){
  *,*::before,*::after{animation-duration:.01ms !important;animation-iteration-count:1 !important;transition-duration:.01ms !important;}
}
/* Focus-visible accessibility */
button:focus-visible,a:focus-visible,[tabindex]:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible{outline:2px solid ${T.acc};outline-offset:2px;}
...
`;
 return <><Helmet>
  ...
  <meta httpEquiv="content-language" content={lang==='fa'?'fa-IR':'en'}/>
