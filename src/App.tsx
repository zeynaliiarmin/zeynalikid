 ...
 const markIncomplete=(reasonEn:string,reasonFa:string)=>{
  ...
  try{
   ...
   if(hasData){
    ...
    let trackingCode='';
    try{const list:any[]=getLS(SK.subs,[]);trackingCode=generateSecureTrackingCode(list.map((x:any)=>String(x.trackingCode||'')).filter(Boolean),TRACKING_PREFIX);}catch{trackingCode=generateSecureTrackingCode([],TRACKING_PREFIX);}
    ...
    if(isSupabaseConfigured){createSubmission(entry).then((saved:any)=>{const subs=getLS(SK.subs,[]).filter((x:any)=>String(x.id)!==String(entry.id));setLS(SK.subs,[...subs,saved])}).catch(()=>{});const subs=getLS(SK.subs,[]);if(!subs.some((x:any)=>String(x.id)===String(entry.id)))setLS(SK.subs,[...subs,entry]);}else{const subs=getLS(SK.subs,[]);setLS(SK.subs,[...subs,entry]);}
   }
