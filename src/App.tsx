...
function ContactPanel({cfg,T,lang,glass}:{cfg:any,T:any,lang:Lang,glass?:boolean}){const c=cfg.contacts||{};const icons=cfg.contactIcons||{};const custom=(c.custom||[]).filter((x:any)=>x.title&&x.url).sort((a:any,b:any)=>(a.order||0)-(b.order||0));
...
function App(){
 ...
 function chooseDest(dest:string,cr:any){const methods=cfg.shippingMethods[dest].filter((m:any)=>m.active).sort((a:any,b:any)=>(a.order||0)-(b.order||0)); const def=methods.find((m:any)=>m.default)||methods[0]; setCourse((c:any)=>({...c,selected:cr,dest,shippingMethod:def?.id||'',form:{...c.form,country:dest==='iran'?(lang==='en'?'Iran':'ایران'):'',receiver:fd.pName,phoneCc:fd.cc,phone:fd.pPhone}})); setShipModal(null); const hasChild=!!(fd.age&&fd.gender); setView(hasChild?'course-shipping':'child-info'); setFlowDeadline(Date.now()+COURSE_TIMER_MS); flowExpiredRef.current=false;}
...
const entry={id:uid(),trackingCode,type:'course',date:today(),time:now(),...data,category:'ثبتی',consultationStatus:'ثبتی',orderStatus:'جدید',priority:autoPriority,unread:true,isNew:true,followReminder:true,followUps:[null,null,null,null,null],adminNotes:'',usageInstructions:'',timeSlot:'',course:course.selected,shipping:{dest:course.dest,method:course.shippingMethod,...course.form,estimatedDelivery:deliveryText(),optionalSendDate:course.optionalSendDate},payment:{...pay,receipt_image:pay.receipt||'',receipt_text:pay.receiptText||'',bank:(cfg.banks||[]).find((b:any)=>b.id===pay.bankId)},childInfo:course.childInfo||null,tonguePhotos:course.tonguePhotos||[],editHistory:course.editedHistory||[],advisor:(()=>{const refC=referralConsultant||(course.advisorId?(cfg.consultants||[]).find((cn:any)=>String(cn.id)===String(course.advisorId)):null);return refC?{id:refC.id,name:refC.name,nameEn:refC.nameEn,referralCode:refC.referralCode}:null;})()}; if(isSupabaseConfigured){try{await createSubmission(entry as any)}catch(e){console.warn('Could not save submission to Supabase, falling back to localStorage',e);reportError('course_register','Could not save submission to Supabase',String((e as any)?.message||e));triggerErrorAlert('registration');const subs=getLS(SK.subs,[]);setLS(SK.subs,[...subs,entry])}}else{const subs=getLS(SK.subs,[]);setLS(SK.subs,[...subs,entry])} setFlowDeadline(null); setCourseResult(entry); clearPublicFormDrafts(); setFd(emptyFd()); setCourse(emptyCourse()); setEditChild(false); setShipModal(null); setView('course-confirm')}
 // ─── انقضای تایمر: ساخت فرم ناقص در پنل + هدایت به صفحهٔ اطلاعات همان دوره (اطلاعات draft حفظ می‌شود) ───
 const expireCourseFlow=()=>{
  if(flowExpiredRef.current)return; flowExpiredRef.current=true;
  try{
   const fp=fullPhone(course.form.phoneCc,course.form.phone);
   const data={...fd,pName:course.form.receiver||fd.pName,cc:course.form.phoneCc,pPhone:course.form.phone,fullPhone:fp};
   let trackingCode='';
   try{const list:any[]=getLS(SK.subs,[]);trackingCode=generateUniqueTrackingCode(list.map((x:any)=>String(x.trackingCode||'')).filter(Boolean),cfg.trackingDigitCount||5);}catch{trackingCode=generateUniqueTrackingCode([],cfg.trackingDigitCount||5);}
   const entry:any={id:uid(),trackingCode,type:'course',date:today(),time:now(),...data,category:'ثبتی ناقص',consultationStatus:'ناقص',orderStatus:'ناقص',incomplete:true,priority:'normal',unread:true,isNew:true,followReminder:false,followUps:[null,null,null,null,null],adminNotes:lang==='en'?'User left the course registration incomplete (15-minute timer expired).':'کاربر روند ثبت دوره را نیمه‌کاره رها کرد (اتمام تایمر ۱۵ دقیقه‌ای).',usageInstructions:'',timeSlot:'',course:course.selected,shipping:{dest:course.dest,method:course.shippingMethod,...course.form,estimatedDelivery:deliveryText(),optionalSendDate:course.optionalSendDate},payment:{...course.payment},childInfo:course.childInfo||null,tonguePhotos:course.tonguePhotos||[],editHistory:course.editedHistory||[],advisor:null};
   if(isSupabaseConfigured){createSubmission(entry).catch(()=>{});const subs=getLS(SK.subs,[]);if(!subs.some((x:any)=>String(x.trackingCode)===String(trackingCode)))setLS(SK.subs,[...subs,entry]);}else{const subs=getLS(SK.subs,[]);setLS(SK.subs,[...subs,entry]);}
  }catch(e){console.warn('expireCourseFlow failed',e);}
  try{if(course.selected?.id)sessionStorage.setItem('zk_course_detail',String(course.selected.id));}catch{}
  setFlowDeadline(null); setView('courses');
...
})()}<ErrorAlertHost cfg={cfg} lang={lang} />{(flowDeadline&&timerViews.includes(view))&&<div style={{position:'fixed',top:showHeader?76:12,left:0,right:0,zIndex:1100,padding:'0 14px',pointerEvents:'none'}}><CourseTimer deadline={flowDeadline} lang={lang}/></div>}<div style={{paddingTop:showHeader?72:0,position:'relative',zIndex:1}}>{page}</div></>;
}
