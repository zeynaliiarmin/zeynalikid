 ...
 const markIncomplete=(reasonEn:string,reasonFa:string)=>{
  ...
  try{
   ...
   if(hasData){
    ...
    let trackingCode='';
    try{const list:any[]=getLS(SK.subs,[]);trackingCode=generateSecureTrackingCode(list.map((x:any)=>String(x.trackingCode||'')).filter(Boolean));}catch{trackingCode=generateSecureTrackingCode([]);}
