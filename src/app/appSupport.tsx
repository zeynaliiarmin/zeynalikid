...
Object.assign(TH, {
 'admin-light':{
  ...
  badge:'#F0FDFA',hdr:'rgba(255,255,255,.62)',sidebar:'#FFFFFF',cardHover:'#F1F5F9',
...
export const StableCountrySelect = memo(function StableCountrySelect({value,onChange,small=true,T,countries,lang}:any){
  ...
  const __btnBd = `1px solid color-mix(in srgb, ${tt.brd} 80%, transparent)`;
  const __btnShadow = `0 3px 8px rgba(0,0,0,.08), 0 1px 2px rgba(0,0,0,.05), inset 0 -1px 0 rgba(255,255,255,.5)`;
  return <Popup open={open} onClose={()=>setOpen(false)} T={tt} width={'33vw'} trigger={<button type="button" onClick={()=>setOpen(v=>!v)} style={{height:50,minWidth:small?86:130,padding:'0 12px',background:tt.inp,border:__btnBd,borderRadius:16,color:tt.accText||tt.acc,cursor:'pointer',fontSize:15,fontFamily:'inherit',fontWeight:800,whiteSpace:'nowrap',order:-1,boxShadow:__btnShadow,transition:'all .2s ease'}}>{shortCountry(list.find((x:any)=>x.code===value)||list[0])}</button>}>{list.map((c:any)=><button key={c.id || c.code} onClick={()=>choose(c.code)} style={{display:'block',width:'100%',padding:'9px 10px',background:value===c.code?tt.soft:'transparent',border:0,borderRadius:9,color:value===c.code?tt.acc:tt.txt,cursor:'pointer',textAlign:'right',fontFamily:'inherit',fontSize:13}}>{labelCountry(c,lang)}</button>)}</Popup>;
});
...
export function Footer({cfg,T,lang,setView,referralConsultant,requestConsult,onStartConsult}:{cfg:any,T:any,lang:Lang,setView:(v:string)=>void,referralConsultant?:any,requestConsult?:()=>void,onStartConsult?:()=>void}){
  ...
  const socialLinks = [
    { key: 'instagram', label: lang==='en'?'Instagram':'اینستاگرام', url: c.instagram ? `https://instagram.com/${String(c.instagram).replace('@','')}` : null, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg> },
    { key: 'telegram', label: lang==='en'?'Telegram':'تلگرام', url: c.telegram ? `https://t.me/${String(c.telegram).replace('@','')}` : null, icon: <PlatformIcon type="telegram" color="#229ED9" size={18}/> },
    { key: 'whatsapp', label: lang==='en'?'WhatsApp':'واتساپ', url: c.whatsapp ? `https://wa.me/${digits(c.whatsapp)}` : null, icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 11.5a8.5 8.5 0 0 1-12.6 7.4L3 20l1.2-4.2A8.5 8.5 0 1 1 20 11.5zM8.5 7.8c.2 3.7 3.1 6.4 6.7 6.8l1-1.7-2.2-1-1 1c-1.3-.5-2.2-1.4-2.8-2.7l1-1-1-2.2-1.7.8z"/></svg> },
  ].filter(s => s.url);
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'28px 16px 18px'}}>
        ...
        <div className="zk-footer-col">
          ...
          <div className="zk-footer-acc-body" data-open={openAcc==='reach'} style={{flexDirection:'column',gap:9,paddingTop:'12px'}}>
            {c.phone && <a href={`tel:${c.phone}`} aria-label={lang==='en'?'Call support':'تماس با پشتیبانی'} style={{display:'flex',alignItems:'center',gap:9,color:T.accText,fontSize:14,fontWeight:600,minHeight:44,padding:'2px 0'}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1.8.3 1.6.6 2.4a2 2 0 0 1-.5 2.1L8 9.4a16 16 0 0 0 6.6 6.6l1.2-1.2a2 2 0 0 1 2.1-.5c.8.3 1.6.5 2.4.6A2 2 0 0 1 22 16.9z"/></svg> <span>{c.phone}</span></a>}
            {c.whatsapp && <a href={`https://wa.me/${digits(c.whatsapp)}`} target="_blank" rel="noreferrer" aria-label={lang==='en'?'WhatsApp support':'پشتیبانی واتساپ'} style={{display:'flex',alignItems:'center',gap:9,color:whatsappAccessible,fontSize:14,fontWeight:600,minHeight:44,padding:'2px 0'}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 11.5a8.5 8.5 0 0 1-12.6 7.4L3 20l1.2-4.2A8.5 8.5 0 1 1 20 11.5zM8.5 7.8c.2 3.7 3.1 6.4 6.7 6.8l1-1.7-2.2-1-1 1c-1.3-.5-2.2-1.4-2.8-2.7l1-1-1-2.2-1.7.8z"/></svg> <span>{lang==='en'?'WhatsApp':'واتساپ'}</span></a>}
            {c.telegram && <a href={`https://t.me/${String(c.telegram).replace('@','')}`} target="_blank" rel="noreferrer" aria-label={lang==='en'?'Telegram support':'کانال تلگرام'} style={{display:'flex',alignItems:'center',gap:9,color:telegramAccessible,fontSize:14,fontWeight:600,minHeight:44,padding:'2px 0'}}><PlatformIcon type="telegram" color={telegramAccessible} size={18}/> <span>{lang==='en'?'Telegram':'تلگرام'}</span></a>}
          </div>
        ...
        <div className="zk-footer-col">
          ...
          {socialLinks.length>0 && <div className="zk-footer-acc-body" data-open={openAcc==='trust'} style={{marginTop:16,gap:10,flexWrap:'wrap',paddingTop:'12px'}}>{socialLinks.map((s,i)=><a key={i} href={s.url!} target="_blank" rel="noreferrer" aria-label={s.label} title={s.label} style={{display:'flex',alignItems:'center',justifyContent:'center',width:42,height:42,borderRadius:999,border:`1px solid ${T.brd}`,background:T.card,color:T.accText}}>{s.icon}<span style={{position:'absolute',width:1,height:1,padding:0,margin:-1,overflow:'hidden',clip:'rect(0,0,0,0)',whiteSpace:'nowrap',border:0}}>{s.label}</span></a>)}</div>}
        </div>
      </div>
      </div>
