...
export default function NotFoundPage(){
 ...
 return <main className={`zk-not-found-page zk-not-found-design-${String(T.id||'classic')}`} style={{...S.page,position:'relative',overflow:'hidden'}} aria-labelledby="not-found-title">
  <Helmet><title>{en?`Page not found | ${brand}`:`صفحه پیدا نشد | ${brand}`}</title><meta name="robots" content="noindex, nofollow"/></Helmet><style>{css}{`.zk-not-found-page:before,.zk-not-found-page:after{content:'';position:absolute;width:280px;height:280px;border-radius:50%;background:${T.soft};filter:blur(42px);opacity:.65;pointer-events:none}.zk-not-found-page:before{top:-90px;inset-inline-start:-100px}.zk-not-found-page:after{bottom:-110px;inset-inline-end:-90px}.zk-not-found-card{animation:fadeSlide .35s ease both}@media(prefers-reduced-motion:reduce){.zk-not-found-card{animation:none}}`}</style>
  <section className="zk-not-found-card" style={{...S.card,maxWidth:620,marginTop:28,textAlign:'center',zIndex:1,borderRadius:T.cardRadius||24,boxShadow:T.shadowStrong||T.neuOut}}>
   <div aria-hidden="true" style={{fontSize:64,fontWeight:900,lineHeight:1,letterSpacing:-3,background:T.grad||T.acc,WebkitBackgroundClip:'text',backgroundClip:'text',color:'transparent'}}>404</div>
   ...
   {items.length>0&&<div style={{margin:'4px 0 18px',textAlign:en?'left':'right'}}><h2 style={{fontSize:16,color:T.ttl,margin:'0 0 10px'}}>{en?'Quick access':'دسترسی سریع'}</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(118px,1fr))',gap:8}}>{items.map((item:any)=><button key={item.title} type="button" onClick={item.run} style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:6,minHeight:88,padding:'9px 8px',background:T.card,border:`1px solid ${T.brd}`,borderRadius:14,boxShadow:T.neuOut,color:T.txt,fontFamily:'inherit',textAlign:'center',cursor:'pointer'}}><span style={{display:'grid',placeItems:'center',width:38,height:38,borderRadius:11,background:T.soft,flexShrink:0}}>{item.icon}</span><span style={{minWidth:0}}><strong style={{display:'block',fontSize:12.5,lineHeight:1.5}}>{item.title}</strong><small style={{display:'block',fontSize:10.5,color:T.mut,lineHeight:1.5}}>{item.desc}</small></span></button>)}</div></div>}
