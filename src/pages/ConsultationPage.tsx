...
function ContactPanelLocal({ cfg, lang, T, publicText, digits }: any) {
  ...
  const darkish = /(?:^|-)dark$/.test(String(T.id||''));
  const shown = darkish ? `color-mix(in srgb, ${color} 62%, white)` : `color-mix(in srgb, ${color} 58%, #0F172A)`; return <a key={i} href={it.url} target={it.url?.startsWith('http') ? '_blank' : undefined} rel="noreferrer" style={{ textDecoration: 'none', padding: '10px 11px', borderRadius: 11, border: `1px solid ${color}66`, background: `${color}14`, color: shown, fontWeight: 700, fontSize: 13, display: 'flex', alignItems: 'center', gap: 7, overflow: 'hidden' }}><PlatformIcon type={it.key} color={shown} /><span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.title}</span></a>; })}</div></div>;
}
