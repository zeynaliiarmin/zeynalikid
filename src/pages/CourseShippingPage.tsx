...
import { PrimaryButton, TextField, TextArea } from '../components/ui/atoms';
...
export default function CourseShippingPage(){
 ...
 return <div dir={lang==='en'?'ltr':'rtl'} style={S.page}><Helmet><title>ثبت‌نام دوره | زینالیکید</title><meta name="description" content="تکمیل ثبت‌نام دوره تخصصی رشد و تغذیه کودکان و نوجوانان" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}</style><div style={{...S.card, paddingTop:'10px'}}>
  <form
    id="zk-course-shipping-form"
    name="course_shipping_form"
    action="/course-shipping"
    method="post"
    data-webmcp-form="course_shipping"
    data-webmcp-purpose="course_shipping_details"
  ...
  </div>{receiverToggle}{receiverSelf?(selfBox):(<Field id="shipReceiver" name="receiver" autoComplete="name" data-webmcp-field="receiver" data-webmcp-label="نام گیرنده" required label={publicText('receiver','نام گیرنده')} value={course.form.receiver} onChange={(v:string)=>setCourse({...course,form:{...course.form,receiver:v}})} ph={publicText('receiver','نام گیرنده')}/>)}<div style={{display:'grid',gridTemplateColumns:'minmax(112px,30%) minmax(0,1fr)',gap:12,alignItems:'start'}}>{course.dest==='iran'?<Field id="shipCity" name="city" autoComplete="address-level2" data-webmcp-field="city" data-webmcp-label="شهر مقصد" required label={publicText('destinationCity','شهر مقصد')} value={course.form.city} onChange={(v:string)=>setCourse({...course,form:{...course.form,city:v}})} ph="تهران، کرج..."/>:<Field id="shipCountry" name="country" autoComplete="country-name" data-webmcp-field="country" data-webmcp-label="کشور مقصد" required label={publicText('destinationCountry','کشور مقصد')} value={course.form.country} onChange={(v:string)=>setCourse({...course,form:{...course.form,country:v}})} ph={lang==='en'?'Country':'کشور'}/>}{receiverSelf?(<div><label style={S.lbl}>{publicText("phone","شماره تماس")}</label><div dir="ltr" style={{fontSize:14,fontWeight:800,color:T.ttl,textAlign:"center"}}>{[course.form.phone, course.form.phoneCc].filter(Boolean).join(" ")}</div></div>) : (<div><label htmlFor="shipPhone" style={S.lbl}>{publicText('phone','شماره تماس')} <span style={{color:T.err}}>*</span></label><div style={{display:'flex',gap:5,direction:'ltr'}}><CountrySelect value={course.form.phoneCc} onChange={(v:string)=>setCourse({...course,form:{...course.form,phoneCc:v}})} countries={countries} T={T} lang={lang}/><TextField id="shipPhone" name="phone" autoComplete="tel" data-webmcp-field="phone" data-webmcp-label="شماره تماس" required dir="ltr" value={course.form.phone} onChange={e=>{const raw=e.target.value;const cleaned=p2e(raw).replace(/[^0-9]/g,'');setCourse({...course,form:{...course.form,phone:cleaned}})}} onPaste={e=>{e.preventDefault();const pasted=e.clipboardData.getData('text');const cleaned=p2e(pasted).replace(/[^0-9]/g,'');setCourse({...course,form:{...course.form,phone:cleaned}})}} placeholder={phonePlaceholder(course.form.phoneCc,lang)} inputMode="numeric" style={{flex:1,minWidth:0}}/></div></div>)}</div>
 {course.dest==='intl'&&<div style={{marginBottom:12}}><label htmlFor="shipWhatsapp" style={S.lbl}>شماره واتساپ یا تلگرام <span style={{color:T.err}}>*</span></label><div style={{display:'flex',gap:5,direction:'ltr'}}>{cfg.whatsappNeedsCountryCode&&<CountrySelect value={course.form.whatsappCc} onChange={(v:string)=>setCourse({...course,form:{...course.form,whatsappCc:v}})} countries={countries} T={T} lang={lang}/>}<TextField id="shipWhatsapp" name="whatsapp" autoComplete="tel" data-webmcp-field="whatsapp" data-webmcp-label="شماره پیام‌رسان" required dir="ltr" value={course.form.whatsapp} onChange={e=>setCourse({...course,form:{...course.form,whatsapp:p2e(e.target.value).replace(/[^0-9+]/g,'')}})} placeholder="xxxxxxxxxxx" inputMode="numeric" style={{flex:1,minWidth:0}}/></div><div style={{fontSize:11,color:T.mut,marginTop:4}}>حتماً با کد کشور وارد شود</div></div>}
...
)}{(method?.requiresPostal||course.dest==='intl')&&<div style={{marginBottom:13}}><label htmlFor="shipPostalCode" style={S.lbl}>{course.dest==='intl'?(/(usa|united states|america|آمریکا|کالیفرنیا|نیویورک|تگزاس|فلوریدا|california|new york|texas|florida)/i.test(String(course.form.country))?publicText('zipCode','ZIP Code'):(course.form.country?trVal('Postal Code'):trVal('Postal Code / ZIP Code'))):trVal('کد پستی')} <span style={{color:T.err}}>*</span></label><TextField id="shipPostalCode" name="postal_code" autoComplete="postal-code" data-webmcp-field="postal_code" data-webmcp-label="کد پستی" required inputMode={course.dest==='iran'?'numeric':undefined} value={course.form.postalCode} onChange={e=>setCourse({...course,form:{...course.form,postalCode:e.target.value}})} placeholder={course.dest==='intl'?(/(usa|united states|america|آمریکا|california|new york|texas|florida)/i.test(String(course.form.country))?publicText('zipCode','ZIP Code'):(course.form.country?trVal('Postal Code'):trVal('Postal Code / ZIP Code'))):trVal('کد پستی')}/></div>}<label htmlFor="shipAddress" style={S.lbl}>{publicText('fullAddress','آدرس کامل')} <span style={{color:T.err}}>*</span></label><TextArea id="shipAddress" name="address" autoComplete="street-address" data-webmcp-field="address" data-webmcp-label="آدرس کامل" required value={course.form.address} onChange={e=>setCourse({...course,form:{...course.form,address:e.target.value}})}/><div style={{background:T.card,borderRadius:14,padding:12,margin:'12px 0',fontSize:12,lineHeight:1.9,boxShadow:T.neuOut}}><b>{publicText('previousInfo','اطلاعات ثبت‌شده قبلی')}</b><br/>{lang==='en'?'Parent name':'نام والد'}: {fd.pName||course.form.receiver||'—'} / {publicText('phone','شماره تماس')}: <span dir="ltr" style={{display:'inline-block'}}>{(course.form.phone?`${course.form.phoneCc||''}${course.form.phone}`:fd.pPhone?`${fd.cc||''}${fd.pPhone}`:'—')}</span>{course.childInfo&&<><br/>{publicText('age','سن')}: {course.childInfo.age||'—'} / {publicText('gender','جنسیت')}: {course.childInfo.gender==='male'?publicText('boy','پسر'):course.childInfo.gender==='female'?publicText('girl','دختر'):'—'} / {lang==='en'?'Course':'دوره'}: {lang==='en'?(course.selected?.titleEn||course.selected?.title):course.selected?.title||'—'}</>}</div>{((cfg.referral||{}).showConsultantSelection===true)&&<div style={{background:T.card,borderRadius:14,padding:12,margin:'12px 0',boxShadow:T.neuOut}}><b style={{display:'block',marginBottom:8,fontSize:13,color:T.ttl}}>{lang==='en'?'Your advisor':'مشاور شما'}</b>{(cfg.consultants||[]).length?<div style={{display:'flex',flexWrap:'wrap',gap:8}}>{(cfg.consultants||[]).map((cn:any)=>{const sel=course.advisorId===cn.id;return <button key={cn.id} type="button" onClick={()=>setCourse((c:any)=>({...c,advisorId:sel?null:cn.id}))} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',borderRadius:999,border:`1px solid ${sel?'var(--zk-primary)':'var(--zk-border)'}`,background:sel?'var(--zk-primary-light)':'var(--zk-surface)',color:'var(--zk-text)',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:700}}>{cn.showPhoto!==false&&(cn.photoUrl||cn.aboutPhotoUrl)?<img src={cn.photoUrl||cn.aboutPhotoUrl} alt="" width={26} height={26} style={{width:26,height:26,borderRadius:'50%',objectFit:'cover'}}/>:null}{lang==='en'?(cn.nameEn||cn.name):cn.name}</button>})}</div>:<span style={{fontSize:11,color:T.mut}}>{lang==='en'?'No advisors configured yet.':'هنوز مشاوری ثبت نشده است.'}</span>}</div>}{Object.keys(course.errors||{}).map((k:string)=><Err key={k} x={course.errors[k]}/>) }<div style={{display:'grid',gridTemplateColumns:'1fr',gap:10,marginTop:12, position:'sticky', bottom: 'calc(12px + env(safe-area-inset-bottom, 0px))', background: T.card, paddingTop:10, paddingBottom: 'calc(8px + env(safe-area-inset-bottom, 0px))', zIndex:10, borderTop: `1px solid ${T.brd}`}}>
  <PrimaryButton type="submit" data-webmcp-action="submit" style={{minHeight:52}}>{publicText('next','ادامه')}</PrimaryButton>
</div>
</form>
{showContactOn('courseShipping')&&<ContactPanel cfg={cfg} T={T} lang={lang}/>}</div></div>
}
