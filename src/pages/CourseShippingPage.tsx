...
export default function CourseShippingPage(){
 ...
 const selfBox = portalSession ? <div style={{marginBottom:12,padding:'11px 13px',borderRadius:13,background:`${T.acc}08`,border:`1px solid ${T.brd}`,textAlign:'center'}}>
  <div style={{fontSize:11,color:T.mut,fontWeight:700,marginBottom:3}}>{lang==='en'?'From your account':'از حساب کاربری شما'}</div>
  <div style={{fontSize:14,color:T.ttl,fontWeight:800}}>{portalSession.fullName||'—'}</div>
  <div dir="ltr" style={{fontSize:13,color:T.mut,fontWeight:700,marginTop:2}}>{portalSession.phone}</div>
 </div> : null;
