...
export default function ChildInfoPage(){
 ...
 {fromConsultForm ? <>
  ...
  <div style={{display:'grid',gridTemplateColumns:'minmax(0,2fr) 110px',gap:12,alignItems:'start',marginBottom:13}}><div><label style={S.lbl}>{publicText('gender','جنسیت')} <span style={{color:T.err}}>*</span></label><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>{([['male',publicText('boy','پسر')],['female',publicText('girl','دختر')] ] as any[]).map((x:any)=>{const isA=draft.gender===x[0];return <button key={x[0]} type="button" className={isA?'zk-chip is-active':'zk-chip'} onClick={()=>onGenderSelect(x[0])} style={{padding:'12px 8px',borderRadius:16,color:isA?T.accText:T.mut,cursor:'pointer',fontSize:14,fontFamily:'inherit',fontWeight:800,minHeight:50}}>{x[1]}</button>;})}</div>{errs.gender && <Err err={errs.gender} theme={T} />}</div><div><Field label={publicText('age',cfg.formFields.age.label)} value={draft.age} onChange={onAgeChange} ph={cfg.formFields.age.placeholder} type="number" required={true} S={S} T={T} trVal={trVal} p2e={p2e} />{errs.age && <Err err={errs.age} theme={T} />}</div></div>
