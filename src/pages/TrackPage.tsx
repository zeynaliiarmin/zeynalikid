...
export default function TrackPage({app}:{app:any}){
 ...
 return <div className="zkgl-root zkgl-has-topbar" dir={lang==='fa'?'rtl':'ltr'} style={{['--zkgl-acc' as any]:T.acc||'#0F766E'}}><Helmet><title>پیگیری ثبت‌نام | زینالیکید</title><meta name="description" content="وارد کردن کد پیگیری و مشاهده وضعیت ثبت‌نام دوره یا فرم مشاوره زینالیکید" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}</style><GlassTopBar brand={lang==='en'?'Zeynalikid':'زینالیکید'} lang={lang} setLang={setLang} onBack={()=>setView('home')} backLabel={lang==='en'?'Back':'بازگشت'} /><div className="zkgl-bg" style={{background:`linear-gradient(150deg, ${T.bg}, ${T.sel||T.soft||T.bg})`}}><svg aria-hidden="true" style={{position:'absolute',inset:0,width:'100%',height:'100%'}} preserveAspectRatio="xMidYMid slice"><circle cx="8%" cy="14%" r="80" fill={mem[0]} opacity=".3"/><circle cx="92%" cy="20%" r="52" fill={mem[1]} opacity=".24"/><circle cx="86%" cy="84%" r="96" fill={mem[2]} opacity=".22"/><circle cx="12%" cy="90%" r="40" fill={mem[0]} opacity=".24"/><path d="M -5 60 Q 25 44 50 60 T 105 60" stroke={mem[1]} strokeWidth="3" fill="none" opacity=".26"/><circle cx="50%" cy="8%" r="4" fill={mem[2]} opacity=".4"/><circle cx="24%" cy="48%" r="3" fill={mem[0]} opacity=".35"/></svg><div style={{position:'absolute',inset:0,backgroundImage:'url(/images/hero-default.webp)',backgroundSize:'cover',backgroundPosition:'center',filter:'blur(7px)',opacity:.4}}/><div style={{position:'absolute',inset:0,background:'linear-gradient(160deg, rgba(15,23,42,.72), rgba(15,23,42,.5))'}}/></div><div className="zkgl-col"><div style={{background:T.card,border:`1px solid ${T.brd}`,borderRadius:20,padding:'24px 20px',boxShadow:T.neuOut,position:'relative',overflow:'hidden'}}><svg aria-hidden="true" style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none'}} preserveAspectRatio="xMidYMid slice"><circle cx="94%" cy="4%" r="46" fill={mem[0]} opacity=".28"/><circle cx="4%" cy="96%" r="30" fill={mem[1]} opacity=".22"/><circle cx="10%" cy="10%" r="10" fill={mem[2]} opacity=".3"/></svg><div style={{position:'relative'}}><h2 style={{color:T.ttl,fontSize:19,fontWeight:900,margin:'0 0 6px'}}>{lang==='en'?'Track your registration':'پیگیری ثبت‌نام'}</h2><p style={{color:T.mut,fontSize:12,lineHeight:1.9,margin:'0 0 18px'}}>{lang==='en'?'Enter your tracking code and the phone number used at registration.':'کد پیگیری و شماره تماسی که هنگام ثبت وارد کردید را وارد کنید.'}</p>
  <label style={S.lbl}>{lang==='en'?'Tracking code':'کد پیگیری'}</label>
  <div dir="ltr" style={{display:'flex',alignItems:'stretch',marginBottom:14}}>
   <span style={{display:'flex',alignItems:'center',padding:'0 12px',background:T.soft,border:`1px solid ${T.brd}`,borderInlineEnd:'none',borderRadius:'12px 0 0 12px',color:T.acc,fontWeight:800,fontFamily:'monospace,-apple-system,"Courier New"',fontSize:16,letterSpacing:'1px',userSelect:'none'}}>ZK</span>
   <input dir="ltr" inputMode="numeric" style={{...S.inp,borderRadius:'0 12px 12px 0',textAlign:'center',letterSpacing:'3px',fontFamily:'monospace,-apple-system,"Courier New"'}} value={num} onChange={e=>onNumChange(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')search()}} placeholder={"1234567890".slice(0,digitCount)} maxLength={8}/>
  </div>
  <label style={S.lbl}>{lang==='en'?'Phone number':'شماره تماس'}</label>
  <div style={{display:'flex',gap:6,direction:'ltr',marginBottom:6}}><input dir="ltr" inputMode="tel" style={{...S.inp,flex:1}} value={phone} onChange={e=>setPhone(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')search()}} placeholder="09123456789"/><button onClick={search} disabled={loading} style={{padding:'0 18px',border:0,borderRadius:12,background:T.grad,color:'#fff',fontWeight:800,cursor:'pointer',fontFamily:'inherit',fontSize:14,opacity:loading?.6:1}}>{loading?'...':(lang==='en'?'Track':'پیگیری')}</button></div>
  <div style={{textAlign:'center',marginTop:12}}>
   <button onClick={enterGuest} style={{padding:'8px 16px',border:`1px dashed ${T.acc}`,borderRadius:12,background:'transparent',color:T.acc,cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:700}}>{lang==='en'?'Guest login':'ورود مهمان'}</button>
  </div>
  {/* اصلاح ۳ (مرحله ۵): پیام خطا دیگر شماره خام واردشده کاربر (تأییدنشده) را نمایش نمی‌دهد */}
  {err&&<div style={{background:`${T.err}12`,border:`1px solid ${T.err}`,borderRadius:11,padding:11,color:T.err,fontSize:12,marginTop:12}}>{err}</div>}
  </div></div>
