      ...
      <div className="zp-content" style={{ justifyContent: 'center', paddingTop: 8 }}>
        <div className="zp-card" style={{ maxWidth: 400 }}>
          ...
          <div className="zp-field">
            ...
            <div className="zp-box"><span className="zp-fic"><svg viewBox="0 0 24 24"><rect x="4" y="11" width="16" height="10" rx="4" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg></span><input ref={pwdRef} className="zp-pwd" dir="ltr" type={showPwd ? 'text' : 'password'} autoComplete="current-password" placeholder="••••••••" value={aPwd} onChange={(e) => setAPwd(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') ok(); }} style={{ letterSpacing: '2px' }} /><button type="button" className="zp-eye" aria-pressed={showPwd} aria-label={en ? (showPwd ? 'Hide password' : 'Show password') : (showPwd ? 'پنهان کردن رمز عبور' : 'نمایش رمز عبور')} onClick={() => setShowPwd(v => !v)}>{showPwd ? <svg viewBox="0 0 24 24"><path d="M3 3l18 18" /><path d="M10.6 5.1A10.9 10.9 0 0 1 12 5c6.5 0 10 7 10 7a17.5 17.5 0 0 1-2.8 3.7M6.6 6.7C3.9 8.4 2 12 2 12s3.5 7 10 7c1.6 0 3-.4 4.2-1" /><path d="M9.9 9.9a3 3 0 0 0 4.2 4.2" /></svg> : <svg viewBox="0 0 24 24"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z" /><circle cx="12" cy="12" r="3" /></svg>}</button></div>
          </div>
