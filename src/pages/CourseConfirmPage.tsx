...
export default function CourseConfirmPage(){
 ...
 const successSentences = isEn
   ? ((cfg.courseSuccessSentencesEn && Array.isArray(cfg.courseSuccessSentencesEn) && cfg.courseSuccessSentencesEn.length > 0) ? cfg.courseSuccessSentencesEn : (cfg.courseSuccessSentences && Array.isArray(cfg.courseSuccessSentences) && cfg.courseSuccessSentences.length > 0 ? cfg.courseSuccessSentences : (cfg.successTrustSentences && Array.isArray(cfg.successTrustSentences) && cfg.successTrustSentences.length > 0 ? cfg.successTrustSentences.map((s:any)=> (s.titleEn || s.title) || s.description || '') : courseSuccessMessages)))
   : ((cfg.courseSuccessSentences && Array.isArray(cfg.courseSuccessSentences) && cfg.courseSuccessSentences.length > 0) ? cfg.courseSuccessSentences : (cfg.successTrustSentences && Array.isArray(cfg.successTrustSentences) && cfg.successTrustSentences.length > 0 ? cfg.successTrustSentences.map((s:any)=> s.title || s.description || '') : courseSuccessMessages));
 const [message]=useState(()=>{try{const used=JSON.parse(sessionStorage.getItem('zkid_used_course_msgs')||'[]');const r=getRandomMessageTracked(successSentences,used);sessionStorage.setItem('zkid_used_course_msgs',JSON.stringify(r.newUsedIndices));return r.message}catch{return successSentences[Math.floor(Math.random()*successSentences.length)] || ('به جمع خانوادهٔ ' + brand + ' خوش آمدید')}});
 return <div style={S.page}><Helmet><title>{`ثبت‌نام دوره | ${brand}`}</title><meta name="description" content="تکمیل ثبت‌نام دوره تخصصی رشد و تغذیه کودکان و نوجوانان" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}</style><div style={S.card}><Stepper step={5}/><h2 style={{color:T.ttl,fontSize:17,fontWeight:800}}>{publicText('finalConfirm','تأیید ثبت‌نام دوره')}</h2><div style={{background:`${T.ok}12`,borderRadius:12,padding:12,marginBottom:12,color:T.ok,fontSize:13,fontWeight:800,boxShadow:T.neuIn}}> {publicText('initialCourseDone','ثبت اولیه دوره انجام شد')}</div>{trackingCode&&<div style={{background:`${T.warn}18`,borderRadius:12,padding:12,marginBottom:12,boxShadow:T.neuIn}}><div style={{fontSize:12,color:T.warn,fontWeight:800,lineHeight:1.9,marginBottom:6}}>{lang==='en'?'Please save or write down your tracking code for the next steps:':'حتماً کد پیگیری را برای مراحل بعدی ذخیره یا یادداشت بفرمایید:'}</div><TrackingCodeBox trackingCode={trackingCode} T={T} lang={lang} label={lang==='en'?'Your tracking code:':'کد پیگیری شما:'} copiedLabel={publicText('copied','کپی شد')} copyLabel={publicText('copy','کپی')}/></div>}<Summary app={app}/><p style={{color:T.mut,fontSize:getTrustFontSize(String(message),13),lineHeight:1.7,margin:'12px 0 0',background:T.soft,borderRadius:12,padding:'9px 11px',boxShadow:T.neuIn,overflow:'hidden'}}>{message}</p><GhostButton style={{marginTop:10}} onClick={()=>setView('courses')}>{lang==='en'?'Register a new course':'ثبت دوره جدید'}</GhostButton>{showContactOn('courseConfirm')&&<ContactPanel cfg={cfg} T={T} lang={lang}/>}</div></div>
}
