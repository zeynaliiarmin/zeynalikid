...
export default function CoursePaymentPage({app}:{app:any}){
 ...
 const cryptoWallets=(advisorWalletArr.length ? advisorWalletArr.map((w:any,wi:number)=>({ id:'aw'+wi, name:w.name||'کیف پول', symbol:w.symbol||'USDT', address:w.address, color:w.color, network:w.network, active:true, order:wi })) : (cfg.cryptoWallets||[])).filter((w:any)=>w&&w.address).sort((a:any,b:any)=>(a.order||0)-(b.order||0));
 ...
 return <div dir={lang==='en'?'ltr':'rtl'} style={{...S.page,minHeight:'100dvh',overflowX:'hidden',padding:'max(6px, env(safe-area-inset-top, 0px)) max(6px, env(safe-area-inset-right, 0px)) max(6px, env(safe-area-inset-bottom, 0px)) max(6px, env(safe-area-inset-left, 0px))',fontFamily:"'Vazirmatn','Tahoma',Arial,sans-serif"}}><Helmet><title>ثبت‌نام دوره | فرزند من</title><meta name="description" content="تکمیل ثبت‌نام دوره تخصصی رشد و تغذیه کودکان و نوجوانان" /><meta name="robots" content="noindex, follow" /></Helmet><style>{css}{payCss}</style>{/* اصلاح ۷: افزایش padding کلی کارت و overflow:auto همیشگی تا با کارت‌های بانکی بزرگ‌تر، محتوا بدون بریدگی و با اسکرول داخلی متوازن نمایش داده شود */}<div style={{...S.card,maxWidth:390,height:'calc(100dvh - 12px)',overflowY:'auto',overflowX:'hidden',padding:10,borderRadius:18,display:'flex',flexDirection:'column',lineHeight:1.3,gap:2}}><Stepper step={4}/>
