...
import { PAYMENT_APP_LAUNCHER_ENABLED } from '../config/project';
...
export default function CoursePaymentPage(){
 ...
 // بررسی امنیتی (کپچا) فقط روی فرم ورود/ثبت‌نام والد در /profile است، نه این‌جا
 ...
 useEffect(()=>{const controller=new AbortController();if(!courseId){setPaymentDetails(current=>current.unlocked||current.loading?{banks:[],wallets:[],loading:false,unlocked:false,error:''}:current);return()=>controller.abort();}const cached=peekCachedPaymentDetails(courseId);setPaymentDetails(cached?{...cached,loading:false,unlocked:true,error:''}:{banks:[],wallets:[],loading:true,unlocked:false,error:''});loadCheckoutPaymentDetails(courseId,referralCode,controller.signal).then(data=>setPaymentDetails({...data,loading:false,unlocked:true,error:''})).catch(error=>{if(error?.name!=='AbortError'){setPaymentDetails({banks:[],wallets:[],loading:false,unlocked:false,error:String(error?.message||'اطلاعات پرداخت در دسترس نیست')})}});return()=>controller.abort()},[courseId,referralCode,reloadAttempt]);
...
<PrimaryButton
  pill
  style={{ marginTop:16, marginBottom:8, padding:'16px', minHeight:56, fontSize:16, flexShrink:0, opacity:payBusy?.72:1, cursor:payBusy?'wait':'pointer'}}
