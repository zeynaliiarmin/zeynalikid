...
import { EXPERIMENTAL_PAYMENT_APP_LAUNCHER, TURNSTILE_SITE_KEY } from '../config/project';
...
export default function CoursePaymentPage(){
 ...
 useEffect(()=>{const controller=new AbortController();if(!courseId||!captchaProof||captchaProof.scope!==captchaScope){setPaymentDetails(current=>current.unlocked||current.loading?{banks:[],wallets:[],loading:false,unlocked:false,error:''}:current);return()=>controller.abort();}setPaymentDetails({banks:[],wallets:[],loading:true,unlocked:false,error:''});loadCheckoutPaymentDetails(courseId,referralCode,captchaProof.token,controller.signal).then(data=>setPaymentDetails({...data,loading:false,unlocked:true,error:''})).catch(error=>{if(error?.name!=='AbortError'){setPaymentDetails({banks:[],wallets:[],loading:false,unlocked:false,error:String(error?.message||'اطلاعات پرداخت در دسترس نیست')});setCaptchaProof(null)}});return()=>controller.abort()},[courseId,referralCode,captchaScope,captchaProof]);
