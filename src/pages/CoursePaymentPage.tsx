...
import { loadCheckoutPaymentDetails } from '../lib/checkoutSession';
...
export default function CoursePaymentPage(){
 ...
 useEffect(()=>{const controller=new AbortController();const courseId=String(course?.selected?.id||'');if(!courseId){setPaymentDetails({banks:[],wallets:[],loading:false});return()=>controller.abort();}setPaymentDetails((current:any)=>({...current,loading:true,error:''}));loadCheckoutPaymentDetails(courseId,String(referralConsultant?.referralCode||''),controller.signal).then(data=>setPaymentDetails({...data,loading:false,error:''})).catch(error=>{if(error?.name!=='AbortError')setPaymentDetails({banks:[],wallets:[],loading:false,error:String(error?.message||'اطلاعات پرداخت در دسترس نیست')})});return()=>controller.abort()},[course?.selected?.id,referralConsultant?.referralCode]);
