...
import { ZkTrashIcon, ZkResetIcon, ZkWarnIcon } from './adminIcons';
...
export default function TrashPanel({T,S,AdminBtn,onRestored,refreshKey}:{T:any,S:any,AdminBtn:()=>any,onRestored:(sub:any)=>void,refreshKey:number}){
 ...
 const handleRestore=async(id:string|number)=>{const sub=deletedSubs.find(s=>s.id===id);if(!sub)return;
  let restored:boolean|undefined;
  try{
   if(isSupabaseConfigured){await restoreSubmission(id)}else{setLS(SK.trash,getLS(SK.trash,[]).filter((x:any)=>x.id!==id));const subs=getLS(SK.subs,[]);const {deleted_at,...clean}=sub;setLS(SK.subs,[...subs,clean])}
   restored=true;
  }catch(e){console.error('Error restoring submission:',e);restored=false}
  // بیرون‌ریزی رکورد از لیست سطل — همیشه بعد از تلاش بازیابی (رفع باگ ماندن ظاهری در سطل)
  if(restored){
   setDeletedSubs(prev=>prev.filter(s=>s.id!==id));
 ...
 return <div>
  <div className="zkad-note-warn"><ZkWarnIcon size={15}/> موارد حذف‌شده تا ۳۰ روز در سطل بازیافت باقی می‌مانند و سپس برای همیشه حذف می‌شوند.</div>
  ...
  {deletedSubs.length>0&&<div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10,padding:'8px 12px',background:T.card,border:`1px solid ${T.brd}`,borderRadius:8}}>
   ...
   {selectedIds.size>0&&<span style={{fontSize:12,color:T.accText,fontWeight:700}}>{selectedIds.size} انتخاب شده</span>}
  </div>}
  ...
  :<div style={{display:'grid',gap:10}}>
   {deletedSubs.map(sub=><div key={sub.id} style={{border:`1px solid ${T.err}`,background:T.badge,borderRadius:14,padding:12}}>
    <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
     ...
     <button title="بازیابی فرم" style={{...AdminBtn(),color:T.ok,borderColor:T.ok,display:'inline-flex',alignItems:'center',gap:6}} onClick={()=>handleRestore(sub.id)}><ZkResetIcon size={14}/> بازیابی</button>
     <button title="حذف دائمی" style={{...AdminBtn(),color:T.err,borderColor:T.err,display:'inline-flex',alignItems:'center',gap:6}} onClick={()=>handlePermanentDelete(sub.id)}><ZkTrashIcon size={14}/> حذف دائمی</button>
    </div>
