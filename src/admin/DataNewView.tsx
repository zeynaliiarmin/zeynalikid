  ...
  return (
    <AdminPopover open={popOpen} onClose={closePop} width={122} ariaLabel={T.en ? 'Phone actions' : 'عملیات شماره تماس'}
      trigger={<button type="button" dir="ltr" aria-haspopup="menu" aria-expanded={popOpen} title={T.en ? 'Call / WhatsApp / Rubika / Copy' : 'تماس · واتساپ · روبیکا · کپی'}
        ...
        style={{ background: 'none', border: 0, padding: 0, margin: 0, cursor: 'pointer', font: 'inherit', fontWeight: 800, fontSize: 12.5, color: T.txt, direction: 'ltr', letterSpacing: '.3px' }}>
        {raw}
      </button>}>
...
export default function DataNewViewPanel({ app }: { app: any }) {
  const { T, subs, filteredAll, statusOptions, getStatus, getPay, changeStatus, changeConsultStatus,
    ...
    nvTab, setNvTab, nvQ, setNvQ, nvPhone, setNvPhone, subCardIO } = app;
  const modalSub = app.modalSub;
  ...
  const codeChip = (code: string) => (
    <button type="button" onClick={(ev) => { ev.stopPropagation(); nvCopy(String(code || '')); }}
      ...
      style={{ background: 'none', border: 0, padding: 0, color: T.accText, fontFamily: 'monospace', fontSize: 12, fontWeight: 900, cursor: 'pointer', textDecoration: 'underline dotted 1px', textUnderlineOffset: 3, direction: 'ltr' }}>
      {code || '—'}
    </button>
  );
  ...
  const card = (g: { key: string; items: any[] }, kind: 'consult' | 'course') => {
    ...
    const rowStyle: CSSProperties = { display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 10, padding: '0 14px' };
    return (
      <div key={kind + g.key} id={'nvc-' + g.key} style={{ border: `1px solid ${nvPhone === g.key ? T.acc : T.brd}`, borderRadius: 14, background: T.card, boxShadow: nvPhone === g.key ? `0 0 0 3px ${T.acc}33` : T.neuOut, marginBottom: 10, overflow: 'hidden', padding: '10px 0 12px' }}>
        <div style={{ ...rowStyle, paddingBottom: 2 }}>
          <b style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13.5, color: T.txt, textAlign: 'start' }}>{dataName(g.key, head)} {!isUser && <span className="zkad-tag" style={{ fontSize: 9.5 }}>{T.en ? 'Guest' : 'مهمان'}</span>}</b>
          <span />
          {codeChip(dataCode(g.key, head))}
        </div>
        <div style={{ ...rowStyle, padding: '6px 14px' }}>
          <input type="checkbox" checked={nvAllSel(nvIds(g.items))} onChange={() => nvToggleIds(nvIds(g.items))} onClick={(ev) => ev.stopPropagation()} style={{ width: 16, height: 16, accentColor: T.acc, cursor: 'pointer', flexShrink: 0, margin: 0 }} aria-label={T.en ? 'Select card' : 'انتخاب کارت'} />
          ...
          <select value={statLbl} onClick={(ev) => ev.stopPropagation()} onChange={(e) => { if (kind === 'consult') changeConsultStatus(head.id, e.target.value); else changeStatus(head.id, e.target.value); }}
            style={{ background: T.inp, border: `1px solid ${T.brd}`, color: T.txt, borderRadius: 8, padding: '4px 7px', fontFamily: 'inherit', fontSize: 11, fontWeight: 800, outline: 'none', cursor: 'pointer', maxWidth: 140 }}>
            {(kind === 'consult' ? consultStatuses : statusOptions).map((s: string) => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
        <div style={{ ...rowStyle, paddingTop: 6, borderTop: `1px dashed ${T.brd}` }}>
          <span style={{ fontSize: 11, color: T.mut, textAlign: 'start' }}>{fmtWhen(head)}</span>
          <button type="button" className="zkad-toolbtn" style={{ fontSize: 11, cursor: 'pointer', justifySelf: 'center' }} onClick={() => openDetails(head, kind)}>{T.en ? 'Details' : 'جزئیات'}</button>
          <span style={{ display: 'inline-flex', gap: 5, flexWrap: 'wrap', justifySelf: 'end' }}>
            {incomplete && <span className="zkad-tag t-warn" style={{ fontSize: 10 }}>ناقص</span>}
            {consulted && <span className="zkad-tag t-ok" style={{ fontSize: 10 }}>{advisorName ? `مشاوره شده توسط ${advisorName}` : 'مشاوره شده'}</span>}
            {hasOther && (
              <button type="button" className="zkad-tag t-info" style={{ fontSize: 10, cursor: 'pointer', border: 0, background: `${T.acc}14`, color: T.accText, padding: '2px 7px', borderRadius: 999 }}
  ...
  const usersCard = (k: string) => {
    ...
    return (
      <div key={'u' + k} style={{ border: `1px solid ${T.brd}`, borderRadius: 14, background: T.card, boxShadow: T.neuOut, marginBottom: 10, padding: '10px 14px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 10 }}>
          <b style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13.5, color: T.txt, textAlign: 'start' }}>{String(u.fullName || '—')}</b>
          <span />
          {codeChip(String(u.code || ''))}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', alignItems: 'center', gap: 10, padding: '7px 0 0' }}>
          <input type="checkbox" checked={nvAllSel([u.id])} onChange={() => nvToggleIds([u.id])} style={{ width: 16, height: 16, accentColor: T.acc, cursor: 'pointer', flexShrink: 0, margin: 0 }} aria-label={T.en ? 'Select user' : 'انتخاب کاربر'} />
          <span style={{ textAlign: 'center' }}><NvPhoneBtn T={T} raw={String(u.fullPhone || ('+' + k))} sub={u} onCopy={nvCopy} /></span>
          <span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, justifySelf: 'end' }}>
            <span className={`zkad-tag ${verified ? 't-ok' : 't-warn'}`} style={{ fontSize: 10 }}>{verified ? (T.en ? 'Verified' : 'تأییدشده') : (T.en ? 'Pending' : 'در انتظار')}</span>
            {hasConsult && <span className="zkad-tag t-warn" style={{ fontSize: 10 }}>فرم مشاوره</span>}
            {hasCourse && <span className="zkad-tag t-info" style={{ fontSize: 10 }}>ثبت دوره</span>}
