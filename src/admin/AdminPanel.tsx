...
const Field=useCallback(({label,value,onChange,ph,type='text',required=false,inputMode}:any)=>{
 ...
 function Admin(){
  ...
  const [nvCounts,setNvCounts]=useState<any[]>([{shown:0,total:0},{shown:0,total:0},{shown:0,total:0}]);
  ...
  // قانون مالک: «ثبت‌نام پنل کاربر» (type:'user') در داشبورد شمرده و نمایش داده نشود.
  const formSubs=subs.filter((x:any)=>x.type==='course'||x.type==='consultation');
  const todaySubs=formSubs.filter((x:any)=> subTime(x) >= startOfToday);
  ...
  const weekAll=formSubs.filter((x:any)=>subTime(x)>=weekAgo).length;
  ...
  const pendingList=formSubs.filter((x:any)=>x.unread||x.isNew||(x.type==='course'&&(x.orderStatus||'جدید')==='جدید')||(x.type==='consultation'&&(x.consultationStatus||'مشاوره اولیه')==='مشاوره اولیه'));
  const activity=useMemo(()=>[...formSubs].sort((a:any,b:any)=>subTime(b)-subTime(a)).slice(0,6).map((x:any)=>({id:x.id,t:subTime(x),tone:x.course?'t-info':'t-ok',txt:`${x.course?'ثبت‌نام دوره':'فرم مشاوره'} — ${x.pName||x.fullPhone||'بدون نام'}`})),[subs]);
 ...
 const navGroups:AdminNavGroup[]=[
  ...
  `}</style><AdminLayout onSave={() => { void setSave(editCfg); }} saving={saveProgress !== null} lang={lang} groups={navGroups} active={aTab} onNavigate={(id:string)=>{setATab(id);setEditCfg(JSON.parse(JSON.stringify(cfg)))}} onLogout={onLogout} onHome={goHome} version="1.0.0">
...
<div className="zkad-data-hero"><div><span className="zkad-data-eyebrow">مدیریت ارتباط با مخاطب</span><h3>فرم‌ها و سفارشات <small title={dataView==='new'?'نمایش‌داده‌شده / کلِ همین تب':'کل ثبت‌شده‌ها / تعداد همین صفحه'} style={{direction:'ltr',display:'inline-block'}}>{dataView==='new'?(faNum((nvCounts[nvTab==='consult'?0:nvTab==='course'?1:2]||{}).shown||0)+(((nvCounts[nvTab==='consult'?0:nvTab==='course'?1:2]||{}).total)!==(nvCounts[nvTab==='consult'?0:nvTab==='course'?1:2]||{}).shown?(' / '+faNum((nvCounts[nvTab==='consult'?0:nvTab==='course'?1:2]||{}).total||0)):' مورد')):(faNum(subs.length)+' / '+faNum(groups.length))}</small></h3></div><button type="button" className="zkad-refresh-btn" onClick={refreshSubmissions} disabled={refreshingSubs}>{refreshingSubs?"در حال بروزرسانی…":"↻ بروزرسانی"}</button></div>
...
<div className="zkad-filter-toggle"><button type="button" onClick={()=>setFiltersOpen(v=>!v)}>{filtersOpen?"− بستن فیلترها":"+ فیلترها و تنظیمات خروجی"}{filtersActive&&<span> فعال</span>}</button>{filtersOpen&&<button type="button" onClick={clearFilters}>پاک‌کردن</button>}</div>{filtersOpen&&<div className="zkad-chips-area">
 ...
 <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,width:'100%'}}><ChipGroup label="دوره" options={coursesF} val={courseF} set={v=>{setCourseF(v);setPage(1)}}/><label className="zkad-toolbtn" style={{display:'inline-flex',alignItems:'center',justifyContent:'center',gap:6}}>فرمت تصویر <select value={imageFormat} onChange={e=>setPersistentImageFormat(e.target.value as any)} style={{border:0,background:'transparent',fontFamily:'inherit',fontWeight:800}}><option value="webp">webp</option><option value="jpg">jpg</option></select></label></div>
  ...
  {selectedCount>0 && <div style={{display:'flex',alignItems:'center',gap:8,marginInlineStart:'auto',flexWrap:'wrap'}}>
    <button type="button" className="zkad-toolbtn zkad-selected-delete" title="حذف انتخاب‌شده‌ها" aria-label="حذف انتخاب‌شده‌ها" onClick={async()=>{ if(!(await zkConfirm(`حذف ${faNum(selectedCount)} مورد انتخاب شده؟`))) return; setSubs((prev:any)=> prev.filter((x:any)=> !selectedIds.has(x.id)),{allowRemoval:true}); clearSelection(); }}><ZkTrashIcon size={16}/></button>
    ...
    <div style={{position:'relative',display:'inline-flex'}}>
      ...
      <div style={{display:'none',position:'absolute',top:'calc(100% + 6px)',insetInlineStart:0,minWidth:220,background:T.pop||'#fff',border:`1px solid ${T.brd}`,borderRadius:12,padding:6,boxShadow:'0 12px 30px rgba(0,0,0,.18)',zIndex:520}} onClick={e=>e.stopPropagation()}>
        <button type="button" className="zkad-toolbtn" style={{width:'100%',justifyContent:'flex-start',border:0,background:'transparent',color:T.txt,padding:'9px 10px'}} onClick={()=>runBackup('subs','excel')}>Excel</button>
        <button type="button" className="zkad-toolbtn" style={{width:'100%',justifyContent:'flex-start',border:0,background:'transparent',color:T.txt,padding:'9px 10px'}} onClick={()=>runBackup('subs','txt')}>txt</button>
        <button type="button" className="zkad-toolbtn" style={{width:'100%',justifyContent:'flex-start',border:0,background:'transparent',color:T.txt,padding:'9px 10px'}} onClick={()=>runBackup('subs','image')}>webp</button>
        <button type="button" className="zkad-toolbtn" style={{width:'100%',justifyContent:'flex-start',border:0,background:'transparent',color:T.txt,padding:'9px 10px'}} onClick={()=>runBackup('subs','pdf')}>pdf</button>
...
{groups.length?groups.map(g=><LazySubCard key={g.head.id} sub={g.head} statusOptions={statusOptions} getStatus={getStatus} onStatusChange={changeStatus} groupCount={g.children.length} allSubs={subs} onOpenRelated={setModalSub} selectedIds={selectedIds} toggleSelect={toggleSelect} isOpen={expId===g.head.id} onToggleOpen={toggleOpenForm} {...subCardIO}/>):<div className="zkad-empty"><ZkSearchIcon size={26}/><p>موردی یافت نشد</p><small>عبارت جستجو یا فیلترها را تغییر دهید</small>{filtersActive&&<button type="button" className="zkad-toolbtn" onClick={clearFilters}><ZkFilterIcon size={14}/> حذف فیلترها</button>}</div>}{modalSub&&<Modal T={T} onClose={()=>setModalSub(null)} max={640}><SubCard sub={modalSub} statusOptions={statusOptions} getStatus={getStatus} onStatusChange={changeStatus} allSubs={subs} onOpenRelated={setModalSub} forceOpen selectedIds={selectedIds} toggleSelect={toggleSelect} {...subCardIO}/></Modal>}<div className="zkad-pager"><button type="button" className="zkad-pager-btn" disabled={safePage<=1} onClick={()=>setPage(p=>Math.max(1,p-1))}>قبلی</button><span className="zkad-pager-cur" title={`صفحه ${safePage} از ${totalPages}`}>{faNum(safePage)}</span><span className="zkad-pager-total">از {faNum(totalPages)}</span><button type="button" className="zkad-pager-btn" disabled={safePage>=totalPages} onClick={()=>setPage(p=>Math.min(totalPages,p+1))}>بعدی</button></div></>:<DataNewViewPanel app={{T,subs,filteredAll,statusOptions,getStatus,getPay,changeStatus,changeConsultStatus,selectedIds,setSelectedIds,toggleSelect,toggleSelectAll,clearSelection,setSubs,setMsg,setMsgType,modalSub,setModalSub,nvTab,setNvTab,nvQ,setNvQ,nvPhone,setNvPhone,subCardIO, setNvCounts}}/>}</>}{aTab==='settings'&&editCfg&&SettingsEditor()}{aTab==='content'&&editCfg&&ContentEditor()}{aTab==='assistant'&&<AssistantManager T={T} S={S} cfg={editCfg||cfg}/>} {aTab==='userQuestions'&&<UserQuestionsEditor app={{...app, AdminBtn, Box, setEditCfg, cfg:editCfg||cfg}}/>}{aTab==='reviews'&&<ReviewsEditor app={{...app, AdminBtn, Box, setEditCfg, cfg:editCfg||cfg}}/>}{aTab==='consultants'&&editCfg&&<ConsultantsEditor T={T} S={S} editCfg={editCfg} setEditCfg={setEditCfg} setSave={setSave} uid={uid} fileToData={fileToData} deleteStoredImage={deleteStoredImage} AdminBtn={AdminBtn} Box={Box} />}{aTab==='contacts'&&editCfg&&ContactsEditor()}{aTab==='courses'&&editCfg&&<CoursesEditor T={T} S={S} editCfg={editCfg} setEditCfg={setEditCfg} setSave={setSave} uid={uid} p2e={p2e} fileToData={fileToData} deleteStoredImage={deleteStoredImage} AdminBtn={AdminBtn} Box={Box} />}{aTab==='featured'&&editCfg&&FeaturedCoursesEditor()}{aTab==='tagged'&&editCfg&&TaggedCoursesEditor()}{aTab==='trust'&&editCfg&&TrustEditor()}{aTab==='trustbox'&&editCfg&&TrustBoxManagerEditor()}{aTab==='images'&&editCfg&&<ImagesManager T={T} S={S} editCfg={editCfg} setEditCfg={setEditCfg} setSave={setSave} uid={uid} fileToData={fileToData} deleteStoredImage={deleteStoredImage} supabase={supabase} isSupabaseConfigured={isSupabaseConfigured} AdminBtn={AdminBtn} />}{aTab==='design'&&editCfg&&DesignManagerEditor()}{aTab==='shipping'&&editCfg&&ShippingBankEditor()}{aTab==='analytics'&&<AnalyticsPanel T={T} S={S}/>}{aTab==='errors'&&<ErrorLogsPanel T={T} S={S}/>}{aTab==='security'&&SecurityEditor()}{aTab==='products'&&editCfg&&ProductsTabEditor()}{aTab==='highlights'&&editCfg&&HighlightsTabEditor(hlCoverCropFor,setHlCoverCropFor,hlBulkTexts,setHlBulkTexts)}{aTab==='licenses'&&editCfg&&LicensesTabEditor()}{aTab==='services'&&editCfg&&ServicesTabEditor()}{aTab==='trash'&&<TrashPanel T={T} S={S} AdminBtn={AdminBtn} refreshKey={trashKey} onRestored={(sub:any)=>{const {deleted_at,...clean}=sub;setSubsState(prev=>prev.some((x:any)=>x.id===clean.id)?prev:[clean,...prev]); if(!isSupabaseConfigured){const subs=getLS(SK.subs,[]); if(!subs.some((x:any)=>x.id===clean.id))setLS(SK.subs,[clean,...subs])}}}/>}{(msg||saveProgress!==null)&&<div style={{position:'fixed',top:16,right:16,zIndex:6000,minWidth:220,maxWidth:320,background:T.pop,border:`1px solid ${msgType==='err'?T.err:(msgType==='ok'?T.ok:T.brd)}`,borderRadius:14,boxShadow:'0 10px 30px rgba(0,0,0,.18)',padding:'12px 14px',animation:'fadeSlide .3s ease both'}}>
  <div
      ...
      onClick={()=>{ try{ setMsg(''); setSaveProgress(null); }catch{} }}
      onPointerDown={(e:any)=>{ try{ e.currentTarget.__sx=e.clientX; e.currentTarget.__sy=e.clientY; }catch{} }}
      onPointerMove={(e:any)=>{ try{ const sx=e.currentTarget.__sx, sy=e.currentTarget.__sy; if(typeof sx==='number'&&(Math.abs(e.clientX-sx)>26||Math.abs(e.clientY-sy)>26)){ setMsg(''); setSaveProgress(null); } }catch{} }}
      onTouchStart={(e:any)=>{ try{ const t=e.touches[0]; e.currentTarget.__sx=t.clientX; e.currentTarget.__sy=t.clientY; }catch{} }}
      onTouchMove={(e:any)=>{ try{ const t=e.touches[0]; const sx=e.currentTarget.__sx, sy=e.currentTarget.__sy; if(typeof sx==='number'&&(Math.abs(t.clientX-sx)>26||Math.abs(t.clientY-sy)>26)){ setMsg(''); setSaveProgress(null); } }catch{} }}
    >
