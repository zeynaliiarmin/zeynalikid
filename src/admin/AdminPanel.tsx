...
export default function AdminPanel(){
 ...
 const navigateFromAssistant=useCallback((tab:string,focus:string)=>{setATab(tab);setEditCfg(JSON.parse(JSON.stringify(cfg)));try{const url=new URL(window.location.href);url.pathname='/admin/app';url.searchParams.set('tab',tab);if(focus)url.searchParams.set('focus',focus);else url.searchParams.delete('focus');window.history.replaceState({},'',url)}catch{}},[cfg,setEditCfg]);
