...
export default function TrustBoxNew({ sentences, interval, T, design, lang }: TrustBoxNewProps) {
  ...
  // زبان — انتخاب عنوان/توضیح بر اساس lang
  const isEn = lang === 'en';
  const curTitle = isEn ? (current.titleEn || current.title) : current.title;
  const curDesc = isEn ? (current.descriptionEn || current.description) : current.description;
  // اندازه فونت پویا — استفاده از توابع کمکی
  const titleSize = getTrustTitleSize(curTitle);
