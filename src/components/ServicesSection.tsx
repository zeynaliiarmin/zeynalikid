...
interface ServiceItem { id: string; title: string; titleEn?: string; description: string; descriptionEn?: string; icon?: string; isVisible?: boolean; isDefault?: boolean; }
...
const serviceIcons: Record<string, React.ReactNode> = {
  ...
  // ۴. تحلیل طبع و مزاج (زبان‌شناسی) — ذره‌بین روی زبان
  'تحلیل طبع و مزاج (زبان‌شناسی)': (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  // English keys for bilingual display
  'Height Growth Activation': (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="23 6 13.5 15.5 8.5 10.5 1 18" />
      <polyline points="17 6 23 6 23 12" />
    </svg>
  ...
  'Weight Control & Height Correction': (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 3v18" />
      <path d="M7 8l5-5 5 5" />
      <rect x="3" y="14" width="18" height="7" rx="2" />
...
const getServiceIcon = (item: ServiceItem, accentColor: string) => {
  const iconSvg = serviceIcons[item.title] || (item.titleEn ? serviceIcons[item.titleEn] : null) || (serviceIcons as any)[item.id] || defaultIconSvg;
  return (
    <div style={{
        ...
        {safeListItems.map((item: ServiceItem) => (
          <div key={item.id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0, textAlign: isRtl ? 'right' : 'left' }}>
              <b style={{ display: 'block', fontSize: 13, color: T.txt, marginBottom: 3, lineHeight: 1.6 }}>{lang==='en' ? (item.titleEn || item.title) : item.title}</b>
              <span style={{ fontSize: 11, color: T.mut, lineHeight: 1.8 }}>{lang==='en' ? (item.descriptionEn || item.description) : item.description}</span>
            </div>
          </div>
        ))}
      </div>
    ...
    >
      ...
      <div style={{
        ...
        {total > 1 && (
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              {/* در فارسی جهت فلش معکوس می‌شود (به سمت راست) */}
        ...
        {total > 1 && (
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              {/* در فارسی جهت فلش معکوس می‌شود (به سمت چپ) */}
      ...
      <div style={{
        ...
        {/*  اصلاح ۶: نقاط در مرکز با جلوگیری از تبدیل به مستطیل عمودی */}
          ...
          {items.map((_: ServiceItem, idx: number) => (
            <button
              ...
              style={{
                ...
                boxShadow: idx === current ? `0 2px 6px ${T.acc}44` : 'none',
              }}
            />
          ))}
        </div>
