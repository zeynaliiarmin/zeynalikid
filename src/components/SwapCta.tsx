...
/**
 * SwapCta — دکمه اصلی با «تعویض نرم متن» روی هاور (دسکتاپ) و تعویض خودکار (موبایل/لمسی).
 ...
 *  • سلسله‌مراتب با رنگ (mode) نه با درخشش دائمی: دکمه ثبت دوره وقتی تب مشاوره باز است
 *    به حالت محوتر (muted) می‌رود و با بسته شدن تب، با انیمیشن به طرح اصلی برمی‌گردد.
 *  • تپش (pulse) فقط در حالت لینک ارجاع یا بعد از کلیک روی دکمه پایین صفحه فعال می‌شود.
 *  • نسخه لمسی: روی دستگاه‌های بدون هاور، متن به‌آرامی و دوره‌ای عوض می‌شود و هنگام لمس متوقف می‌شود.
 *  • احترام کامل به prefers-reduced-motion.
 */
interface SwapCtaProps {
  /** نوع بصری دکمه */
  variant?: 'consult' | 'enroll';
  /** سلسله‌مراتب رنگی دکمه ثبت‌نام (primary = طرح اصلی، muted = محو وقتی مشاوره در کانون توجه است) */
  mode?: 'primary' | 'muted';
