...
export default function HamburgerMenu({T,lang,setLang,cfg,publicText,APP_A_URL,setView,referralConsultant,referralTarget,findTabByCode,onCoursesClick,onConsultClick}:Props){
 ...
 const entryMode=String((cfg as any)?.entryMode||'user');
 ...
 const itemsBase:Record<string,{label:string; icon:React.ReactNode; to?:string; fn?:()=>void; separator?:boolean}>=Object.fromEntries([
  ...
  ['track',{label: portalEntryLabel, icon: entryMode==='user' ? <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.acc} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4" /><path d="M4.5 21c1.4-3.8 4.2-5.8 7.5-5.8s6.1 2 7.5 5.8" /></svg> : <TrackIcon size={20} color={T.acc} />, to:entryMode==='user'?'/portal':'/track', separator: true}],
