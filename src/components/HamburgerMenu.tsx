...
export default function HamburgerMenu({T,lang,setLang,cfg,publicText,APP_A_URL,setView,referralConsultant,referralTarget,findTabByCode,onCoursesClick,onConsultClick}:Props){
 ...
 return <>
  ...
  <aside aria-label={lang==='fa'?'منوی اصلی':'Main menu'} style={{position:'fixed',top:0,right:isRtl?0:'auto',left:isRtl?'auto':0,bottom:0,zIndex:1500,width:'min(52vw,235px)',background:T.pop,borderTopLeftRadius:isRtl?20:0,borderBottomLeftRadius:isRtl?20:0,borderTopRightRadius:isRtl?0:20,borderBottomRightRadius:isRtl?0:20,boxShadow:isRtl?'-18px 0 48px rgba(15,38,60,.18)':'18px 0 48px rgba(15,38,60,.18)',display:'flex',flexDirection:'column',padding:'0 12px calc(12px + env(safe-area-inset-bottom, 0px))',overflowY:'auto',overflowX:'hidden',direction:isRtl?'rtl':'ltr',animation:`${isRtl?'zk-public-menu-in-rtl':'zk-public-menu-in-ltr'} .32s cubic-bezier(.16,1,.3,1) both`}}>
   ...
   {items.map((it:any)=>{const active=!!it.to&&location.pathname===it.to; const coursePulse = it._id==='courses'&&(referralConsultant||referralTab); return <div key={it._id}>{it.separator&&<div style={{height:1,margin:'8px 12px',background:T.brd}}/>}{it.to?<Link to={it.to} onClick={()=>setOpen(false)} style={coursePulse?{...itemStyleFor(active),fontWeight:900,color:'var(--zk-primary)',animation:'zk-menu-pulse 1.6s ease-in-out infinite'}:itemStyleFor(active)}><span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}}>{it.icon}</span><span>{it.label}</span>{active&&<span aria-hidden="true" style={{marginInlineStart:'auto',color:"var(--zk-primary)",display:'flex',alignItems:'center',justifyContent:'center',width:22,height:22,borderRadius:8,background:'rgba(122,18,212,.08)'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--zk-primary)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{transform: isRtl ? 'scaleX(-1)' : 'none', display:'block'}}><polyline points="9 18 15 12 9 6"/></svg></span>}</Link>:<button type="button" onClick={go(it.fn!)} style={coursePulse?{...itemStyleFor(false),fontWeight:900,color:'var(--zk-primary)',animation:'zk-menu-pulse 1.6s ease-in-out infinite'}:itemStyleFor(false)}><span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}}>{it.icon}</span><span>{it.label}</span></button>}{it._id==='home'&&<div style={{height:1,margin:'8px 12px',background:T.brd}}/>}</div>})}
  </aside></>}
