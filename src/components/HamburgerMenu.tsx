...
export default function HamburgerMenu({T,lang,setLang,cfg,publicText,APP_A_URL,setView,referralConsultant,referralTarget,findTabByCode,onCoursesClick,onConsultClick}:Props){
 ...
 return <>
  ...
  {open&&<><div aria-hidden="true" onMouseDown={()=>setOpen(false)} style={{position:'fixed',inset:0,zIndex:1400,background:'rgba(15,30,45,.34)',backdropFilter:'blur(2px)'}}/>
  <aside aria-label={lang==='fa'?'منوی اصلی':'Main menu'} style={{position:'fixed',top:0,right:isRtl?0:'auto',left:isRtl?'auto':0,bottom:0,zIndex:1500,width:'min(52vw,235px)',background:T.pop,borderTopLeftRadius:isRtl?20:0,borderBottomLeftRadius:isRtl?20:0,borderTopRightRadius:isRtl?0:20,borderBottomRightRadius:isRtl?0:20,boxShadow:isRtl?'-18px 0 48px rgba(15,38,60,.18)':'18px 0 48px rgba(15,38,60,.18)',display:'flex',flexDirection:'column',padding:'0 12px calc(12px + env(safe-area-inset-bottom, 0px))',overflowY:'auto',overflowX:'hidden',direction:isRtl?'rtl':'ltr'}}>
  ...
  <style>{`@keyframes zk-menu-pulse{0%,100%{box-shadow:0 0 0 0 rgba(15,118,110,.4)}50%{box-shadow:0 0 0 8px rgba(15,118,110,0)}}`}</style>
 </>;
}
