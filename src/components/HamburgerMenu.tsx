...
import { getUserSession } from '../utils/userPortal';
...
export default function HamburgerMenu({T,lang,setLang,cfg,publicText,APP_A_URL,setView,referralConsultant,referralTarget,findTabByCode,onCoursesClick,onConsultClick}:Props){
 ...
 const coursesLabel = isDirectCourse
   ? (lang==='en' ? `View details & register ${directCourseName}` : `مشاهده جزئیات و ثبت ${directCourseName}`)
   : referralTab
   ? (lang==='en' ? `View ${referralTab.titleEn||referralTab.title} courses` : `مشاهده دوره‌های ${referralTab.title}`)
   : publicText('menuCourses','معرفی دوره‌ها');
 ...
 const entryMode=String((cfg as any)?.entryMode||'track');
 ...
 const itemsBase:Record<string,{label:string; icon:React.ReactNode; to?:string; fn?:()=>void; separator?:boolean}>=Object.fromEntries([
  ...
  ['track',{label: portalEntryLabel, icon: <TrackIcon size={20} color={T.acc} />, to:'/track', separator: true}],
  ['profile',{label: lang==='en'?'My Profile':'پروفایل من', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.acc} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>, to:'/profile'}],
 ...
 const isRtl=lang==='fa';
 return <>
  <button type="button" onClick={()=>setOpen(true)} aria-label={lang==='fa'?'باز کردن منو':'Open menu'} aria-expanded={open} style={{position:'fixed',top:'calc(8px + env(safe-area-inset-top, 0px))',[isRtl?'right':'left']:'max(16px, env(safe-area-inset-'+(isRtl?'right':'left')+', 0px))',[isRtl?'left':'right']:'auto',zIndex:1300,background:"var(--zk-surface)",border:"1px solid var(--zk-border)",borderRadius:12,padding:0,color:"var(--zk-text)",cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',width:48,height:48,opacity:open?1:.94,boxShadow:"var(--zk-shadow-light)",transition:'all .2s ease'}}><MenuIcon size={22} color={T.acc} /></button>
  {open&&<><div aria-hidden="true" onMouseDown={()=>setOpen(false)} style={{position:'fixed',inset:0,zIndex:1400,background:'rgba(15,30,45,.34)',backdropFilter:'blur(2px)',animation:'zk-public-menu-overlay .24s ease both'}}/>
  <aside aria-label={lang==='fa'?'منوی اصلی':'Main menu'} style={{position:'fixed',top:0,right:isRtl?0:'auto',left:isRtl?'auto':0,bottom:0,zIndex:1500,width:'min(52vw,235px)',background:T.pop,borderTopLeftRadius:isRtl?20:0,borderBottomLeftRadius:isRtl?20:0,borderTopRightRadius:isRtl?0:20,borderBottomRightRadius:isRtl?0:20,boxShadow:isRtl?'-18px 0 48px rgba(15,38,60,.18)':'18px 0 48px rgba(15,38,60,.18)',display:'flex',flexDirection:'column',padding:'0 12px calc(12px + env(safe-area-inset-bottom, 0px))',overflowY:'auto',overflowX:'hidden',direction:isRtl?'rtl':'ltr',animation:`${isRtl?'zk-public-menu-in-rtl':'zk-public-menu-in-ltr'} .32s cubic-bezier(.16,1,.3,1) both`}}>
   <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,minHeight:64,height:`calc(64px + env(safe-area-inset-top, 0px))`,padding:'calc(env(safe-area-inset-top, 0px)) 4px 0',borderBottom:`1px solid ${T.brd}`,marginBottom:10,boxSizing:'border-box'}}><strong style={{fontSize:17,color:T.ttl}}>{String(cfg.browserTitle||cfg.siteTitle||(lang==='fa'?'منو':'Menu')).replace(/[“”"]/g,'').trim()}</strong><button type="button" onClick={()=>setOpen(false)} aria-label={isRtl?'بستن منو':'Close menu'} style={{width:36,height:36,minHeight:36,border:"1px solid var(--zk-border)",borderRadius:10,background:T.soft,color:"var(--zk-primary-text)",cursor:'pointer',fontSize:20,display:'flex',alignItems:'center',justifyContent:'center',lineHeight:1,fontFamily:'inherit',padding:0}}>×</button></div>
   ...
   {items.map((it:any)=>{const active=!!it.to&&location.pathname===it.to; const coursePulse = it._id==='courses'&&(referralConsultant||referralTab); return <div key={it._id}>{it.separator&&<div style={{height:1,margin:'8px 12px',background:T.brd}}/>}{it.to?<Link to={it.to} onClick={()=>setOpen(false)} style={coursePulse?{...itemStyleFor(active),fontWeight:900,color:'var(--zk-primary-text)',animation:'zk-menu-pulse 1.6s ease-in-out infinite'}:itemStyleFor(active)}><span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}}>{it.icon}</span><span>{it.label}</span>{active&&<span aria-hidden="true" style={{marginInlineStart:'auto',color:"var(--zk-primary-text)",display:'flex',alignItems:'center',justifyContent:'center',width:22,height:22,borderRadius:8,background:'rgba(122,18,212,.08)'}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--zk-primary)" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" style={{transform: isRtl ? 'scaleX(-1)' : 'none', display:'block'}}><polyline points="9 18 15 12 9 6"/></svg></span>}</Link>:<button type="button" onClick={go(it.fn!)} style={coursePulse?{...itemStyleFor(false),fontWeight:900,color:'var(--zk-primary-text)',animation:'zk-menu-pulse 1.6s ease-in-out infinite'}:itemStyleFor(false)}><span style={{display:'flex',alignItems:'center',justifyContent:'center',width:24,height:24}}>{it.icon}</span><span>{it.label}</span></button>}{it._id==='home'&&<div style={{height:1,margin:'8px 12px',background:T.brd}}/>}</div>})}
  </aside></>}
  <style>{`@keyframes zk-public-menu-overlay{from{opacity:0}to{opacity:1}}@keyframes zk-public-menu-in-rtl{from{opacity:.45;transform:translateX(100%)}to{opacity:1;transform:translateX(0)}}@keyframes zk-public-menu-in-ltr{from{opacity:.45;transform:translateX(-100%)}to{opacity:1;transform:translateX(0)}}@keyframes zk-menu-pulse{0%,100%{box-shadow:0 0 0 0 rgba(15,118,110,.4)}50%{box-shadow:0 0 0 8px rgba(15,118,110,0)}}`}</style>
 </>;
}
