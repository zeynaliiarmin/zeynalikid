/**
 * SSR لبهٔ Vercel برای ربات‌ها (Dynamic Rendering استاندارد گوگل).
 * همین دامنه است، پس Content-Type دقیق text/html با بدنهٔ کامل HTML برمی‌گردد.
 * داده از همان public-settings سوپابیس خوانده می‌شود — بدون دیپلوی و بدون دیتای تکراری.
 */
...
async function loadSettings(): Promise<Record<string, any>> {
  const r = await fetch(SUPABASE_FN, { headers: { 'Accept': 'application/json' } });
  const d: any = await r.json();
  return (d?.settings || {}) as Record<string, any>;
}

export default async function handler(req: any, res: any) {
  const slug = String((req.query?.slug ?? '') as string).replace(/^\/+|\/+$/g, '').trim();
  try {
    const settings = await loadSettings();
    const brand = String(settings.browserTitle || settings.siteTitle || 'سامانه').replace(/[“”"]/g, '').trim();
    if (!slug) {
      res.redirect(302, `${SITE_BASE}/education`);
      return;
    }
    ...
    if (!item) {
      res.status(404).send(`<!doctype html><html dir="rtl" lang="fa"><head><meta charset="utf-8"><title>محتوا یافت نشد | ${esc(brand)}</title><meta name="robots" content="noindex"><meta http-equiv="refresh" content="2;url=${SITE_BASE}/education"></head><body><p>این محتوا یافت نشد. <a href="${SITE_BASE}/education">بازگشت به آموزش‌ها</a></p></body></html>`);
      return;
    }
    ...
    const canonical = `${SITE_BASE}/education/${encodeURIComponent(seoKeyOf(item))}`;
    ...
    const sourceLink = item.sourceUrl && /^https?:\/\//.test(String(item.sourceUrl))
      ? ` · <a href="${esc(String(item.sourceUrl))}" rel="noreferrer" target="_blank" style="color:#0B5D56">منبع</a>` : '';
...
<nav style="padding:10px 0"><a href="${SITE_BASE}/education" style="color:#0B5D56;text-decoration:none">← بازگشت به آموزش‌ها</a></nav>
<article itemscope itemtype="https://schema.org/Article">
<h1 itemprop="headline">${esc(title)}</h1>
<p style="color:#4b5563;font-size:13px;margin:0 0 12px">${esc(String(item.author || brand))}${item.date ? ' · ' + esc(String(item.date)) : ''}${sourceLink}</p>
...
<a href="${SITE_BASE}/form" style="display:inline-block;background:#0F766E;color:#fff;padding:10px 20px;border-radius:12px;text-decoration:none;font-weight:700">درخواست مشاوره</a>
</section>
</main></body></html>`;
